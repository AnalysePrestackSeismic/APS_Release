...
write_vol=1 ; % Make this parameter 0 if you dont want write out the connectted bodies as volume 
...
if TF == 0
    ...
    if isfield(job_meta, 'anom_result')
        ...
        while lpi <= loopfin
            ...
            n_slices = (end_sample-start_sample)+1;
            ...
            if lpi == 1;
                I = single(zeros(n_slices,pkeyn*skeyn));
            end
            
            I(1:n_slices,double(lin_ind)) = traces(start_sample:end_sample,:);
            fprintf('-- Block %d --\n',lpi);
            lpi = lpi + 1;
            % fprintf('-- Block %d --\n',lpi);
