 function [] = node_slurm_submit_cloud(altnodes,mcr_root,mcr_cache_root,run_script_path,algorithm_name,job_meta_path,slurm_part,n_cores,varargin)
%% ------------------ Disclaimer  ------------------
% 
...
if strcmp(slurm_part,'Blades_Test')
    fprintf(fid_batch, 'MCR_CACHE_ROOT=%s/%s\n',mcr_cache_root,randdir); 
else
    fprintf(fid_batch, 'MCR_CACHE_ROOT=%s/%s\n',mcr_cache_root,randdir); 
end
%fprintf(fid_batch, 'MCR_CACHE_ROOT=/localcache/Paradigm/jonesce/%s\n',randdir);
fprintf(fid_batch, ['MCRROOT=%s;\n',mcr_root]);
