function [slices] = node_binary_read_slices_lite_SW(seismic,i_block,n_blocks,binary_slice_path,window_length)
...
count_files = sprintf('ls -B %ssas_result_slices_block_*.bin | wc -l',binary_slice_path);
...
start_slice_idx = (1+((i_block-1)*floor(seismic.n_samples/n_blocks)))-window_length;
...
if i_block == n_blocks
    end_slice_idx = window_length+seismic.n_samples;
    %     end_slice_idx = window_length+ns;
else
    end_slice_idx = window_length+(i_block*floor(seismic.n_samples/n_blocks));
...
if end_slice_idx >= seismic.n_samples
    % if end_slice_idx >= ns
    pad_end = zeros(seismic.n_traces,end_slice_idx-seismic.n_samples);
    end_slice_idx = seismic.n_samples;
    %     end_slice_idx = ns;
end
...
for ii = 1:n_files
    ...
    file_to_open = sprintf('%ssas_result_slices_block_%d.bin',binary_slice_path,ii);
