function[average,variance] = sas_stats_empirical3(data_file_in,time_file_in,bytespersample,nt,ntwin,olap,run_number)
%
...
%           run_number = used to track file names. 
...
%% THIS IF STATEMENT IS FAILING FOR SOME REASON
    ...
    if notnumbermean(t2) - notnumbermean(t1) > 0.0039
...
% x = downsample1(cumu(ntwin/2:end-ntwin/2),win_shift);
% 
% xi = 1:1:nt;
% 
% fprintf('\nx...\n');
...
fid3 = fopen(sprintf('%d_ecdf_%s',run_number,data_file_in),'a');
...
for i = 1:nt
    ...
    fprintf('Time Slice: No:')
  i
    %Read a slice from data_file_in.
    data(:,1) = fread(fid1,ntrace1,'float32'); 
    %Calculate the ecdf probability for all points in the volume. This gives the CDF at all values in data, the problem is when the value is the same it only calculates this once. 
    ...
    [emp_prob,emp_pos] = ecdf1(data);
     
    %This data now needs re-sorted back to the true data. 
    
    % define a vector with the indices of every trace
    g = 1:ntrace1;   
    % find the indices of the minimum elements 
    
    
    MIN = min(data);
    a   = find(data == min(data));
    % remove the location of the minimum elements from the index vector 
    g(a) = [];
    resort_prob(a,1) = 0;  
       
    totalnumberofloops = (ntrace1-length(a))*nt; 
    % now loop through the old data to find all other (non minimum) data values.
    
    for k = 1:(ntrace1-length(a))    
       
       a =  data(g(1,k),1);      
      
       if isnan(data(g(1,k),1)) ~= 1 
                     
       value = find(emp_pos == data(g(1,k),1));

       resort_prob(g(k),1) = emp_prob(value); 
       
       else
      
       resort_prob(g(k),1) = NaN;
       
       end
              
       end
            
    % Now each point has a probability need to differentiate between below mean and above mean.  
    [row1, col1] = find(resort_prob<0.5);
    [row2, col2] = find(resort_prob>=0.5);
    [row3] = isnan(resort_prob);
    
    prob1 = (2 * (1-resort_prob(row1))) - 1;
    prob2 = (2 * resort_prob(row2)) - 1; 
    prob3 = (resort_prob(row3));
    
    prob4(row1,1) = prob1;
    prob4(row2,1) = prob2;
    prob4(row3,1) = 1e30;
    
    % plot(data,prob3,'+',data,prob5,'o')
    % datastore.normal = prob5;
    % %      frame=getframe;
    % %      writeVideo(writerObj,frame);    
    % %      end
       
    % Write the anomally indicator value to file.
    fwrite(fid3,prob4,'float32');
...
% plot(datastore.normal,datastore.cumulative,'+');
