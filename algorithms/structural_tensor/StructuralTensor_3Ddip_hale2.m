function [vol_3d_orig shifts] = StructuralTensor_3Ddip_hale2
...
dipx = reshape(atand(nx./nz),nt,nxl,nil);
dipy = reshape(atand(ny./nz),nt,nxl,nil);
...
shifts = zeros(nsamp,1);
%data = make_data(nsamp,nt,nxl,nil,nx,ny,nz,planarity,eta,shifts);
%planarity = reshape(planarity,nt,nxl,nil);  
figure(2)
subplot(1,3,1); imagesc(squeeze(vol_3d_orig(:,:,5)))
%subplot(1,3,2); imagesc(squeeze(planarity(:,:,5)))
subplot(1,3,3); imagesc(squeeze(dipy(:,:,5)),[-90 90])
% start with shift field equal to zero
disp('Calculate shifts')
% nonlinear approach of lomask
for kk = 1:itermax
    % Calculate residual
    r = calcr(nsamp,nt,nxl,nil,nx,ny,nz,planarity,eta,shifts);
    
    % Calculate shift
    [dshifts] = pcg(@(x)calcshifts(x,nsamp,nt,nil,nxl,nx,ny,nz,planarity,eta),...
        [calculate_dx(nt,nxl,nil,'no_transp');calculate_dy(nt,nxl,nil,'no_transp');eta.*calculate_dz(nt,nxl,nil,'no_transp')]'...
        *spdiags([planarity;planarity;repmat(eta,nsamp,1)],0,3*nsamp,3*nsamp)'*r,tol,maxit,[],[]);
    
    shifts = shifts+dshifts;
    pause(0.1)
    ra
end
...
function plot_shifts(nt,nxl,nil,vol_3d_orig,nx,ny,nz,shifts,fignum)
    z_cube = repmat(1:nt,nxl,1)';
    z_cube = repmat(z_cube,1,nil);
    z_cube = reshape(z_cube,nt,nxl,nil);
    nx = reshape(nx,nt,nxl,nil); 
    ny = reshape(ny,nt,nxl,nil); 
    nz = reshape(nz,nt,nxl,nil);   

    shifts = reshape(shifts,nt,nxl,nil);

    figure(fignum)
    subplot(3,1,1); imagesc(squeeze(vol_3d_orig(:,:,5))); title('Inline');
    subplot(3,1,2); imagesc(squeeze(z_cube(:,:,5))-squeeze(shifts(:,:,5)),[0 nt])
    %subplot(1,4,3); imagesc((squeeze(nx(:,:,5))/squeeze(nz(:,:,5))))
    subplot(3,1,3); imagesc(squeeze(z_cube(:,:,5)),[0 nt])
    
    figure(fignum+1)
    subplot(3,1,1); imagesc(squeeze(vol_3d_orig(:,5,:))); title('Crossline');
    subplot(3,1,2); imagesc(squeeze(z_cube(:,5,:))-squeeze(shifts(:,5,:)),[0 nt])
    %subplot(1,4,3); imagesc((squeeze(nx(:,:,5))/squeeze(nz(:,:,5))))
    subplot(3,1,3); imagesc(squeeze(z_cube(:,5,:)),[0 nt])
    
    figure(fignum+2)
    subplot(3,1,1); imagesc(squeeze(vol_3d_orig(20,:,:))); title('Slice');
    subplot(3,1,2); imagesc(squeeze(z_cube(20,:,:))-squeeze(shifts(10,:,:)),[0 nt])
    %subplot(1,4,3); imagesc((squeeze(nx(:,:,5))/squeeze(nz(:,:,5))))
    subplot(3,1,3); imagesc(squeeze(z_cube(20,:,:)),[0 nt])
end


function r = calcr(nsamp,nt,nxl,nil,nx,ny,nz,planarity,eta,shifts)
%     r = (spdiags([planarity;planarity;repmat(eta,nsamp,1)],0,3*nsamp,3*nsamp)...
%         *[calculate_dx(nt,nxl,nil,'no_transp');calculate_dy(nt,nxl,nil,'no_transp');calculate_dz(nt,nxl,nil,'no_transp')])'...
%         *(spdiags([planarity;planarity;repmat(eta,nsamp,1)],0,3*nsamp,3*nsamp)...
%         *(([calculate_dx(nt,nxl,nil,'no_transp');calculate_dy(nt,nxl,nil,'no_transp');calculate_dz(nt,nxl,nil,'no_transp')]*shifts)-[nx./nz;ny./nz;zeros(nsamp,1)]));
    r = ([calculate_dx(nt,nxl,nil,'no_transp');calculate_dy(nt,nxl,nil,'no_transp');eta.*calculate_dz(nt,nxl,nil,'no_transp')]...
        *shifts)-[nx./nz;ny./nz;zeros(nsamp,1)];

end

function data = make_data(nsamp,nt,nxl,nil,nx,ny,nz,planarity,eta,shifts)
...
function y = calcshifts(x,nsamp,nt,nil,nxl,nx,ny,nz,planarity,eta) 
%     y = (spdiags([planarity;planarity;repmat(eta,nsamp,1)],0,3*nsamp,3*nsamp)...
%         *[calculate_dx(nt,nxl,nil,'no_transp');calculate_dy(nt,nxl,nil,'no_transp');calculate_dz(nt,nxl,nil,'no_transp')])'...        
%         *(spdiags([planarity;planarity;repmat(eta,nsamp,1)],0,3*nsamp,3*nsamp)...
%         *[calculate_dx(nt,nxl,nil,'no_transp');calculate_dy(nt,nxl,nil,'no_transp');calculate_dz(nt,nxl,nil,'no_transp')]*x);    
    
    y = ([calculate_dx(nt,nxl,nil,'no_transp');calculate_dy(nt,nxl,nil,'no_transp');calculate_dz(nt,nxl,nil,'no_transp')]'...
        *spdiags([planarity;planarity;repmat(eta,nsamp,1)],0,3*nsamp,3*nsamp)'...
        *spdiags([planarity;planarity;repmat(eta,nsamp,1)],0,3*nsamp,3*nsamp)...
        *[calculate_dx(nt,nxl,nil,'no_transp');calculate_dy(nt,nxl,nil,'no_transp');calculate_dz(nt,nxl,nil,'no_transp')])*x;
