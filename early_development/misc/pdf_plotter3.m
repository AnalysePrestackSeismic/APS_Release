...
for c=1:numplot
    ...
    [hdrtext hdrvalue] = textread(file_in,'%s%s','delimiter',':');  %read the headers in the file
    hdrvalue_tmp = str2double(hdrvalue);
    while isnan(hdrvalue_tmp(end,1))
        hdrvalue_tmp = hdrvalue_tmp(1:end-1,1);
    end
    hdrvalue = hdrvalue(1:length(hdrvalue_tmp),1);
    [pdf] = textread(file_in,'','headerlines',header);              %read the pdf matrix in the file
    
    x_min = hdrvalue_tmp(end-6,1);    %set minimum x value defined in the headers
    x_int = hdrvalue_tmp(end-5,1);    %set x bin size defined in the headers
    y_min = hdrvalue_tmp(end-2,1);    %set minimum y value defined in the headers
    y_int = hdrvalue_tmp(end-1,1);    %set y bin size defined in the headers
    
    pdf = pdf(1:hdrvalue_tmp(end-4,1),1:hdrvalue_tmp(end,1));
...
%     y_axis = fliplr(y_axis);
    ...
    if savepdf > 0
%         fid1 = fopen(sprintf('column_format_%s',file_in),'wt');
%         fprintf(fid1,'Probability Density Function 2D output by Jonathan Edgar Matlab code\n\n');
%         fprintf(fid1,'%s\t\t\t',hdrvalue{2});
%         fprintf(fid1,'%s\t\t',hdrvalue{6});
%         fprintf(fid1,'Probability\n');
%         for x = 1:nx
%             for y = 1:ny
%                 fprintf(fid1,'%f\t%f\t%f\n',x_min+(x-1)*x_int,y_min+(y-1)*y_int,pdf(y,x));
%             end
%         end
