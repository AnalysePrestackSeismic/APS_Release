function [] = angle_stack(seismic_mat_path,i_block,n_blocks,seis_ext,output_dir,sample_rate,shift,zsmooth,mute)
...
%     end
    ...
    %results_out{3,2} = zeros(n_samples,1);
...
%        results_out{1,2}{2,3}(trace_counter,:) =  uint32(1);     
        ...
        trim_data = traces(:,start_idx:end_idx);
...
%         results_out{4,2}(:,trace_counter) = trim_sum;
        ...
        if size(mute,1) > 1 
            for i_mute = 1:1:size(mute,1)
                results_out{1+i_mute,1} = strcat(filename,'ang_mute_',num2str(mute{i_mute,1}),'-',num2str(mute{i_mute,2}));
                results_out{1,2}{1,0+i_mute}(trace_counter,:) = ilxl_read(start_idx,:);
                results_out{1,2}{2,0+i_mute}(trace_counter,:) = uint32(1);
            end
            ...
            for i_mute = 1:1:size(mute,1)
                ...
                if sum(mute_mask) ~= 0
                    results_out{1+i_mute,2}(:,trace_counter) = (1/sum(mute_mask))*sum(trim_data(:,mute_mask),2);  % avg
                else
                    results_out{1+i_mute,2}(:,trace_counter) = zeros(n_samples,1);
                end
