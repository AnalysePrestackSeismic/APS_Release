...
ref(100,1)=0.25;  %makes multiple causing point, multiple
class(200,1)=1;
...
if tvw == 1
    for ii = 1:length(angles) %for every angle gather
        ...
        w_tmp = {ricker(ricker_cf(ii),ricker_sampling),tvw_att(ii)*ricker(ricker_cf_att(ii),ricker_sampling)};   %has wavelet for angle set stored as original and attenuated in array cells 1 and 2
...
if coloured_noise == 1
    ...
    m_amp(231,5)=-0.16;
    m_amp(242,6)=-0.18;
    %second, deeper, half amplitude
    m_amp(300,1)=-0.04; 
    m_amp(304,2)=-0.05;
    m_amp(310,3)=-0.06;
    m_amp(321,4)=-0.07;
    m_amp(333,5)=-0.08;
    m_amp(344,6)=-0.09;m_amp(300,1);
