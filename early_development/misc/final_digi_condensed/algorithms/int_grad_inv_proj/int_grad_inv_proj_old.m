...
% Load wavelet set
wavelet = load(strcat(job_meta.wav_directory,'all_wavelets_time.mat'));
...
% Read traces for this i_block from each of the angle stacks
for ii = 1:seismic.nfiles
    seismic.filepath = strcat(seismic.input_dir,seismic.input_filenames{1,1}{ii});
    %[traces{ii}, ilxl_read{ii}] = node_segy_read_traces_lite(seismic,i_block,n_blocks,dec);
    [seismic, traces, ilxl_read, offset_read] = node_segy_read_traces_and_gathers(seismic,i_block,n_blocks,dec);
end


% Flatten seismic to water bottom
file_to_open = fopen(strcat(seismic.output_dir,'waterbottom/waterbottom_idx_block_',num2str(i_block),'.bin'));
wb_idx = fread(file_to_open,'float32')';
fclose(file_to_open);
...
for kk = first_iter:last_iter
    ...
    chi = seismic.srate*(0:1:ns-1)'.*-2 + 19;
