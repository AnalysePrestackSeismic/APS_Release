function [] = ava_analysis( job_meta_path,block,int_dir,grad_dir,maxz,win_chi,inc_chi,int_dp_lim,grad_dp_lim,post_process_flag)
%% ------------------ Disclaimer  ------------------
% 
...
cpa_directory = strcat(job_meta.output_dir,'crossplot_analysis','_',maxz,'_',win_chi,'_',inc_chi,'_',int_dp_lim,'_',grad_dp_lim,'_',post_process_flag,'/'); % The name of the crossplot analysis directory
...
chi_z_emperical=(z_axis-(job_meta.s_rate/1000)*job_meta.wb_z_avg(block))*chi_slope/1000 +chi_int ;   % Emperical chi trend used in DIGI start model and EER projection, depth calculated below mudline
flag_lim=0;
...
for ic=1:(nwin_chi-1)
    ...
    if (max(abs(intercept_ic_uw_raw))>0 && max(abs(gradient_ic_uw_raw))>0)
        ...
        if(sum(ll)>(percentile_keep/100)*nsamples_th)
            [int_mean_z(ic),grad_mean_z(ic),int_std_z(ic),grad_std_z(ic),slope_z(ic),~,~, chi_z(ic), dist_z(ic)]=ava_statistics(intercept_ic_uw(ll),gradient_ic_uw(ll),2);        % Calculate ava stats from all points in intercept and gradient
            if crossplot==1
                figure(1);
            ...
            %-----------------project intercept------------------
            grad_trace_p_proj =gradient_ic_uw(int_sample_p);                                                % Take the gradient value for extrema of intercept trace positions(for peaks)
            grad_trace_t_proj =gradient_ic_uw(int_sample_t);                                                % Take the gradient value for extrema of intercept trace positions (for troughs)
            ...
            if(sum(ll_pts)>(percentile_keep/100)*nsamples_th*0.1)
                [int_mean_z_pts(ic),grad_mean_z_pts(ic),int_std_z_pts(ic),grad_std_z_pts(ic),slope_z_pts(ic),~,~, chi_z_pts(ic),dist_z_pts(ic)]=ava_statistics(int_trace_pts(ll_pts),grad_trace_pts(ll_pts),2);
                flag_check(ic)=0;
...
close all;
clear ic nans_pts int_trace_p int_trace_t int_trace_pts grad_trace_p_proj grad_trace_t_proj grad_trace_pts;
clear intercept_ic gradient_ic intercept_ic_uw gradient_ic_uw intercept_temp gradient_temp ll ll_p ll_t ll_pts crossplot int grad int_sample_p int_sample_t;
...
loc= job_meta.block_keys(block,:);                  % get the geometry of the block
results_AVA.inl_central= floor((loc(1)+loc(2))/2);  % Calculate the central inline
results_AVA.xl_central = floor((loc(3)+loc(4))/2);  % Calculate the central crosspline
...
results_AVA.data_pts=[int_mean_z_pts;grad_mean_z_pts;int_std_z_pts;grad_std_z_pts;slope_z_pts;chi_z_pts;dist_z_pts];        % Write various results into one matrix (for peaks and troughs)
...
results_AVA.data_t=[int_mean_z_t;grad_mean_z_t;int_std_z_t;grad_std_z_t;slope_z_t;chi_z_t;dist_z_t];                        % Write various results into one matrix (for troughs)


%% Interpolate result to seismic vertical scale
z_axis_2=(1:maxzout)*job_meta.s_rate/1000;
results_AVA_interp=struct ;
results_AVA.zaxis=z_axis_2;
results_AVA_interp.data=[interp1(z_axis,results_AVA.data',z_axis_2,'cubic')]';
results_AVA_interp.data_pts=[interp1(z_axis,results_AVA.data_pts',z_axis_2,'cubic')]';
results_AVA_interp.data_p=[interp1(z_axis,results_AVA.data_p',z_axis_2,'cubic')]';
results_AVA_interp.data_t=[interp1(z_axis,results_AVA.data_t',z_axis_2,'cubic')]';
...
% Store results
save(strcat(cpa_directory,'crossplot_analysis_block',num2str(block),'.mat'),'-struct','results_AVA_interp','-v7.3');
...
if plot_result==1
    ...
    for plot_inc=1:7
        ...
        plot(results_AVA_interp.data(plot_inc,:),z_axis_2,'-r','LineWidth',3); set(gca,'YDir','rev');                                % Plot depth trend of all points in red
        hold on;
        plot(results_AVA_interp.data_pts(plot_inc,:),z_axis_2,'-b','LineWidth',3); set(gca,'YDir','rev');                            % Plot depth trend of all peaks and troughs in blue
        hold on;
        plot(results_AVA_interp.data_p(plot_inc,:),z_axis_2,'-g','LineWidth',3); set(gca,'YDir','rev');                            % Plot depth trend of all peaks in green
        hold on;
        plot(results_AVA_interp.data_t(plot_inc,:),z_axis_2,'-m','LineWidth',3); set(gca,'YDir','rev');                            % Plot depth trend of all peaks in magenta
        
        
        ylim([min(z_axis_2) max(z_axis_2)]);ylabel('depth/time');
        switch  plot_inc
            case 1
