function[averagei,stddevi,val] = sas_stats_edit(data_file_in,time_file_in,bytespersample,nt,ntwin,olap,run_number)
%
...
%% THIS IF STATEMENT IS FAILING FOR SOME REASON
    ...
    if nanmean(t2) - nanmean(t1) > 0.0039
...
average(1) = nanmean(data);
variance(1) = nanvar(data); 
...
for k = 1:nloop   
    ...
    average(k+1) = nanmean(data);
    variance(k+1) = nanvar(data); 
...
fid3 = fopen(sprintf('%s_cdf',data_file_in),'a');
...
for i = 1:nt
    ...
    a(:,1) = cdf1('Normal',data,averagei(i),stddevi(i));
 
      
    for k = 1:99
        b = k*0.01;
    n(i,k) = findnearest(b,a);
    val(i,k) = data(n(i,k),1);
    end
    ...
    prob = (2*cdf('Normal',abs(data-averagei(i))+averagei(i),averagei(i),stddevi(i)))-1;
