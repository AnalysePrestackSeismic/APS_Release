function [] = int_grad_inv_proj_par(job_meta_path,i_block,startvol,volinc,endvol)
% -------------------------------------------------------------------------
...
%
% Authors: James Selvage, Jonathan Edgar and a bit of Charles Jones and Simon Wriggley
% -------------------------------------------------------------------------
%% Parameters
...
% number of traces to run, put to zero to make it run all traces in the block, this is the default
tottracerun = 0;
%tottracerun = 200;
...
% do you want to calculate and output standard line fitting intercept and
% gradient and eer
output_std = 0;

% do you want interactive plots to pop up when running (just for debug)
plot_on = 0;
...
% do you want a confidence volume calculating and outputing, 0 = no , 1 = yes
needconf = 0;

% how many % progress reports do you want in the running of the job, 5 = 20% increment for reporting
noofreports = 20;

% what is the chi model that you want for the eer
...
%  Weight for EER constraint between 0 and 1, value of 1 forces it much
%  closer to the intercept values
eer_weight = 0.1;

% maximum number of iterations in the lsq solver 
iter = 300;

% convergence tolerance in the lsq solver can move to tol = 5e-4 or 1e-4
% but larger value speeds it up as less iterations to run
tol = 1e-3;
...
%cjmodel = rand(4100,1);
%
% now loop round all the traces in th einput and run the inversion we have
