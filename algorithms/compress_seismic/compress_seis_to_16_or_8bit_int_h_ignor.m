function [ trim_data_filt, scalars_out, zlocs_out, tpgrad_orig, scalepeak,  orig_nsamples, percenterr ] = compress_seis_to_16_or_8bit_int_g( trim_data, morecompress, envlplot )
%
%% ------------------ Disclaimer  ------------------
%
...
for ckk = 1:no_of_traces
    ...
    if sum(trim_data(:,ckk)) ~= 0;
        ...
        % need to stop this being manual, instead just keep making envelopes
        % until the number of points is less than the number of storage
        % points and then add in the higher priority points
        ...
        %         figure(789);
        %         plot(mposenv,'-b'); hold on
        %         plot(tmposenv,'--m'); hold off
        %         figure(788);
        %         plot(mitpslocsin,mitpsval,'-b'); hold on
        %         plot(itpslocsin,itpsval,'-b');
        %         plot(titpslocsin,titpsval,'--m');
        %         %plot(tmitpslocsin,tmitpsval,'--m');
        %         hold off;
        ...
        [tupdiflocs, tupdifval] = diffenvlop(newposvals, origuprval,firstpos, endpos, intrlen, origuprloc, nanperwhtnoiseb);
        ...
        if isempty(updiflocs) ~= 1
            ...
            figure(1001);
            plot(tupdiflocs,tupdifval,'-ob');
            hold on;
            plot(updiflocs,updifval,'--og');
            hold off;
        ...
        updiflocs = updiflocs(totalenvdistval > diffpercentcut);
        updifval = updifval(totalenvdistval > diffpercentcut);
        ...
        %         lowdiflocs = lowdiflocs(totalenvdistval > diffpercentcut);
        %         lowdifval = lowdifval(totalenvdistval > diffpercentcut);
        ...
        %calc the envelope of the revised 3rd envelope to know
        %which are the points that have to be kept
        max_1st_deriv = diff(itpsval);
        max_2nd_deriv = diff(itpsval,2);
        sign_1_deriv = sign(max_1st_deriv);
        sign_1_deriv(sign_1_deriv == 0) = 1;
        diffsign = diff(sign_1_deriv);
        diffsign(sign(max_2nd_deriv) > 0) = 0;
        diffsign = [-1;diffsign;-1];
        %keep all points where second derivative is less than zero
        %diffsign = diffsign + [-1;max_2nd_deriv;-1];
        keepuprval = itpsval(diffsign < 0);
        keepuprloc = itpslocsin(diffsign < 0);
        
        % make sure the start and end do not get lost
        if keepuprloc(end) ~= intrlen
            keepuprloc = [keepuprloc; intrlen];
            keepuprval = [keepuprval; endpos];
        end
        if keepuprloc(1) ~= 1
            keepuprloc = [1; keepuprloc];
            keepuprval = [firstpos; keepuprval];
        end
        
        % so those are the upper vals to keep
        % now need the lower vals
        ...
        mdiffsign = diff(sign_1_deriv);
        mdiffsign(sign(max_2nd_deriv) <= 0) = 0;
        mdiffsign = [1;mdiffsign;1];
        %keep all points where second derivative is less than zero
        %mdiffsign = mdiffsign + [1;max_2nd_deriv;1];
        keeplowval = mitpsval(mdiffsign > 0);
        keeplowloc = mitpslocsin(mdiffsign > 0);
        
        %         figure(281);
        %         %plot(itpsval,'-or');
        %         plot(mitpsval,'-or');
        %         hold on;
        %         %itpsval([1;max_2nd_deriv;1] > 0) = 0;
        %         mitpsval([1;max_2nd_deriv;1] < 0) = 0;
        %         %plot(itpsval,'ob');
        %         plot(mitpsval,'ob');
        %         plot([1;(max_1st_deriv)],'og');
        %         plot([1;(max_2nd_deriv)],'om');
        %         plot(zeros(size(mitpsval,1),1),'-k');
        %         hold off;
        
        % make sure the start and end do not get lost
        if keeplowloc(end) ~= intrlen
            keeplowloc = [keeplowloc; intrlen];
            keeplowval = [keeplowval; endpos];
        end
        if keeplowloc(1) ~= 1
            keeplowloc = [1; keeplowloc];
            keeplowval = [firstpos; keeplowval];
        end
        
        
        % now add any points that the greater then 180 degrees, which is
        % probably a change in the sign of the second derivative
        
        %if envlplot == 10
        figure(55); plot(keepuprloc,keepuprval,'-or'); hold on;
        plot(keeplowloc,keeplowval,'-om');
        plot(mitpslocsin,mitpsval,'-b')
        plot(itpslocsin,itpsval,'-g')
        %plot(posenv,'-r')
        %plot(mposenv,'-g');
        plot(trim_data(1:noofrealsamps,ckk),'-k');
        %hold off;
        ...
        % now add up the total number of points in those two envelopes and
        % that leaves how many then to select to add to them
        ...
        bigcoef = sortrows([abs([9999999;9999999;totaldist_red(3:(end-2));9999999;9999999])  points_all ],-1);
...
%==========================================================================
    ...
    function [fupdiflocs, fupdifval] = diffenvlop(fnewposvals, foriguprval,d_firstdif, d_enddif, d_trc_len, foriguprloc, fnanperwhtnoiseb)
        % calc the envelope of the difference
        ...
        ddiffsign(sign(dmax_2nd_deriv) > 0) = 0;
        %ddiffsign = [-1;ddiffsign;-1];
        fupdifval = foriguprval([-1;ddiffsign;-1] < 0);
        fupdifval = fupdifval + abs(fnewposvals([-1;ddiffsign;-1] < 0).*fnanperwhtnoiseb);
        fupdiflocs = foriguprloc([-1;ddiffsign;-1] < 0);
        % make sure the start and end do not get lost
    ...
    function [newlocs, newvals] = addpoints(origlocs,origvals,newlocs,newvals,fullorigcount)
        % add the two sets of points together
        if isempty(newlocs) ~= 1
            ...
            newlocs = fullorigcount(tmpmerge ~= fullorigcount);
            newvals = tmpmerge(tmpmerge ~= fullorigcount);
        end
    end

    function [toriglocs, torigvals] = outsidetest(toriglocs,torigvals,tnewlocs,tnewvals,florigcountb)
        %test to see if any points have been chopped off in the envelope
        ...
        if (size(florigcountb,1) == size(toriglocs,1))
            torigvals = torigvals(tnewvals < torigvals);
            toriglocs = toriglocs(tnewvals < torigvals);
        else
            fnewposvals =  newinterp(toriglocs);
            dtmpupper = florigcountb;
            dtmpupper(tnewlocs) = tnewvals;
            dtmpupper(toriglocs(fnewposvals < torigvals)) = torigvals(fnewposvals < torigvals);
