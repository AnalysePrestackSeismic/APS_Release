function [] = wavelet_estimationandQC(job_meta_path,i_block,decimate,first_live_block)
%% ------------------ Disclaimer  ------------------
%
...
if job_meta.is_gather == 0
    ...
    if isfield(job_meta, 'wb_path')
        ...
        % nov 2015 change to make logic on the wbpick lookup consitant,
        % rational is that the gather out of node segy read is allways full
        % fold
        traces{vol_index_wb} = gathers(:,pick_wb_ind:length(offset):end);
    else
...
for i_vol = 1:1:i_vol_max
    if job_meta.is_gather == 0
        ...
        traces = gathers(:,i_vol:length(offset):end);
    end
