function [] = wavelet_estimationandQC_EGY(job_meta_path,i_block,decimate,first_live_block)
%% ------------------ Disclaimer  ------------------
%
...
for i_vol = 1:1:i_vol_max
    ...
    if n_traces > 2
        if size(traces,2)<size(win_ind,2) % for EGY WDDM 2014 datset to handle holes in data
            win_ind=win_ind(:,1:size(traces,2));
        end
