function [ trim_data_filt, scalars_out, zlocs_out, tpgrad_orig, scalepeak,  orig_nsamples, percenterr ] = compress_seis_to_16_or_8bit_int_k5( trim_data, morecompress, envlplot )
%
%% ------------------ Disclaimer  ------------------
%
...
finaldroptri = 0.002;
%nanperwhtnoise  = 0.02;
diffpercentcut_orig = 0.1; %1.5; % this is the difference that it starts to drop at so bigger is more points dropped
diffpercentcut = diffpercentcut_orig;
diffpercentcut_update = 0.05; % this needs to vary with the rate of update
diffpercentcut_max = 2;
nearfirst = 0;
fdiffaddmul = 0.25;  % the amount of the cutt off to add to the data each loop
nanperwhtnoisecf = 0.1;
...
elseif morecompress == 2
    ...
    %    nanperwhtnoise  = 0.007; %0.02; % 0.007;  % % to add to avoid cliping
    %    nanperwhtnoisediff = 0.007;
    nanperwhtnoise  = 0; %0.02; % 0.007;  % % to add to avoid cliping
    nanperwhtnoisediff = 0;
    nanperwhtnoisec = 0;
    %perwhtnoise = 0.06;
...
for ckk = 1:no_of_traces
    ...
    if sum(trim_datacj(:,ckk)) ~= 0;
        ...
        %itpsval = itpsval + abs(itpsval.*nanperwhtnoisecf);
        itpsval = itpsval + abs(median(itpsval)*nanperwhtnoisecf);
        
        %was adding some white noise
        %mitpsval = mitpsval - abs(mitpsval.*nanperwhtnoisecf);
        mitpsval = mitpsval - abs(median(mitpsval)*nanperwhtnoisecf);
        ...
        for envl = 1:nl
            ...
            if envl < 3
                ...
                origlowloc = mitpslocsin;
                origlowval = mitpsval;
                %fprintf('set the original envelope\n');
            else
                % keep the points from the orignal envelope and use new
                % values at those points instead, so that test of the
                % difference is done at all the points that orginal peaks
                % interpolate to make the envelope only using fast linear interp
                prevuppr = double(makefastinterp1(double(itpslocsin),double(itpsval),double(origuprloc)));
                %figure(87); plot(origuprloc,prevuppr,'-ob'); hold on;plot(origuprloc,origuprval,'-or'); 
                origuprval(prevuppr > origuprval) = prevuppr(prevuppr > origuprval);  % upper envelope
                %plot(origuprloc,origuprval,'--xg');

                % lower envelope
                prevlwr = double(makefastinterp1(double(mitpslocsin),double(mitpsval),double(origlowloc)));
                %figure(88); plot(origlowloc,prevlwr,'-ob'); hold on;plot(origlowloc,origlowval,'-or'); hold off
                %plot(origlowloc,prevlwr,'-ob');plot(origlowloc,origlowval,'-or');
                origlowval(prevlwr < origlowval) = prevlwr(prevlwr < origlowval);  % upper envelope
                %plot(origlowloc,origlowval,'--xg');
                %title('blue - interpolated last iteration, red starting iteration, green dash, updated last iteration'); hold off;
            end
            %==========================================================================
            % upper envelope
            ...
            newnegvalsb =  newnegvalsall(origlowloc);
            newposvalsb =  newposvalsall(origuprloc);
            ...
            [updiflocs, updifval] = diffenvlop(newposvals, origuprval,firstpos, endpos, intrlen, origuprloc, nanperwhtnoisediff, newposvalsb, diffpercentcut, 0);
            ...
            [itpslocsin, itpsval] = outsidetest(origuprloc,origuprval,itpslocsin,itpsval,origcount,0,nanperwhtnoise,fdiffaddmul);
            ...
            [updiflocs, updifval] = diffenvlop(newnegvals, origlowval,firstneg, endneg, intrlen, origlowloc, nanperwhtnoisediff, newnegvalsb, diffpercentcut, 1);
            ...
            % add in the missed off points from the orginal envelope, but only those which are positive inflection
            [mitpslocsin, mitpsval] = outsidetest(origlowloc,origlowval,mitpslocsin,mitpsval,origcount,1,nanperwhtnoise,fdiffaddmul);
            ...
            if envlplot == -2
                ...
                plot(itpslocsin,itpsval,'-or');
                plot(mitpslocsin, mitpsval,'-or');
                plot(updiflocs, updifval,'xb');
                plot(trim_data(1:noofrealsamps,ckk),'-k')
                plot(mposenv,'-m');
                plot(posenv,'-m');
                title(strcat(sprintf('loop no %d',envl),' green - orginal envelope; magenta - this iteration envelope, red this envelope with large errors removed, ie points added back in; blue before fixing missed points envelope'));
                hold off;
            end
            %=========================================================================
            % now test to see if points left are just linear interpolations
            % of points either side, take first derivative and look either
            % side
            %mitpslocsin,mitpsval
            gradtotest = diff(itpsval,1)./diff(itpslocsin,1);
            %figure(47); plot(itpslocsin,itpsval,'-or'); hold on; %plot(itpslocsin,[1;gradtotest],'-b');
            %itpsvalplt = itpsval.*0;
            %itpsvalplt(logical([0;round(diff(gradtotest,1)*100) == 0;0])) = itpsval(logical([0;round(diff(gradtotest,1)*100) == 0;0]));
            %itpsvalplt(logical([abs([10000000;diff(gradtotest,1)]) < abs(gradtotest*0.000001);0])) = itpsval(logical([abs([10000000;diff(gradtotest,1)]) < abs(gradtotest*0.000001);0]));
            %gradtotest(abs([10000;diff(gradtotest,1)]) < abs(gradtotest*0.000001))
            %plot(itpslocsin,itpsvalplt,'gs','MarkerFaceColor','g','MarkerSize',10);plot(itpslocsin,itpsval,'-or');
            
            itpslocsin = itpslocsin(logical([abs([10000000;diff(gradtotest,1)]) > abs(gradtotest*gradperval);1]));
            itpsval = itpsval(logical([abs([10000000;diff(gradtotest,1)]) > abs(gradtotest*gradperval);1]));
            %plot(itpslocsin,itpsval,'gs','MarkerFaceColor','g','MarkerSize',10);
            %hold off;
            %itpslocsin,itpsval
            gradtotest = diff(mitpsval,1)./diff(mitpslocsin,1);
            %figure(48); plot(mitpslocsin,mitpsval,'-or'); hold on; % plot(mitpslocsin,[1;gradtotest],'-b');
            %mitpsvalplt = mitpsval.*0;
            %mitpsvalplt(logical([0;round(diff(gradtotest,1)*100) == 0;0])) = mitpsval(logical([0;round(diff(gradtotest,1)*100) == 0;0]));
            
            mitpslocsin = mitpslocsin(logical([abs([10000000;diff(gradtotest,1)]) > abs(gradtotest*gradperval);1]));
            mitpsval = mitpsval(logical([abs([10000000;diff(gradtotest,1)]) > abs(gradtotest*gradperval);1]));
            
            %plot(mitpslocsin,mitpsval,'gs','MarkerFaceColor','g','MarkerSize',10);%plot(mitpslocsin,mitpsval,'-or');
            %hold off;            
            %gradtotest = [1;diff(mitpsval,1)./diff(mitpslocsin,1)];
            %figure(47); plot(gradtotest,'-or'); hold on; plot(mitpsval,'-b');
            
            %=========================================================================
            
            % add in the missed off points from the orginal data, but only those which are positive inflection
            [mitpslocsinck, ~] = outsidetest_data(origcount,trim_data(1:noofrealsamps,ckk),mitpslocsin,mitpsval,origcount,1,(nanperwhtnoise),firstneg,endneg,intrlen);
            
            % add in the missed off points from the orginal data, but only those which are positive inflection
            [itpslocsinck, ~] = outsidetest_data(origcount,trim_data(1:noofrealsamps,ckk),itpslocsin,itpsval,origcount,0,(nanperwhtnoise),firstpos,endpos,intrlen);
            ...
            % decide if time to exit the loop and check on rate of change
            % in number of points
            if (size(mitpslocsinck,1) + size(itpslocsinck,1)) < floor(tpgrad);
%                fprintf('under limit no of points %d ; no of points used %d\n',floor(tpgrad),(size(mitpslocsinck,1) + size(itpslocsinck,1)));
                % add in the missed off points from the orginal data, but only those which are positive inflection
                [mitpslocsin, mitpsval] = outsidetest_data(origcount,trim_data(1:noofrealsamps,ckk),mitpslocsin,mitpsval,origcount,1,(nanperwhtnoise),firstneg,endneg,intrlen);
                % add in the missed off points from the orginal data, but only those which are positive inflection
                [itpslocsin, itpsval] = outsidetest_data(origcount,trim_data(1:noofrealsamps,ckk),itpslocsin,itpsval,origcount,0,(nanperwhtnoise),firstpos,endpos,intrlen);
                break
                
            elseif  (size(mitpslocsinck,1) + size(itpslocsinck,1)) < floor(tpgrad)*1.3;
                if nearfirst == 1
                    ...
                    if lastdiff - (size(mitpslocsinck,1) + size(itpslocsinck,1)) < (tpgrad*0.05)
                        diffpercentcut = diffpercentcut + (diffpercentcut_update*2);
                    else
                        diffpercentcut = diffpercentcut + (diffpercentcut_update/2);
                    end
...
%                fprintf('near limit no of points; diffcut %f ; no of points used %d\n',diffpercentcut,(size(mitpslocsinck,1) + size(itpslocsinck,1)));
                
            else
%                fprintf('limit no of points %d ;diffcut %f ; no of points used %d\n',floor(tpgrad),diffpercentcut,(size(mitpslocsinck,1) + size(itpslocsinck,1)));
                % if the difference between the last update and this one is small boost the update increase
                if lastdiff - (size(mitpslocsinck,1) + size(itpslocsinck,1)) < (tpgrad*0.05)
                    diffpercentcut = diffpercentcut + (diffpercentcut_update*2);
                else
            ...
            lastdiff = size(mitpslocsinck,1) + size(itpslocsinck,1);
            
            %             %=========================================================================
            %             % old version which added the missing points back on each loop
...
%    if sum(trim_data(:,ckk)) ~= 0;
        ...
        if or(and( percenterr(ckk) > 0.25, morecompress > 2), and( percenterr(ckk) > 0.1, morecompress <= 2))
...
%[updiflocs, updifval] = diffenvlop(newposvals, origuprval,firstpos, endpos, intrlen, origuprloc, nanperwhtnoise, newposvalsb, diffpercentcut, 0);
    function [fupdiflocs, fupdifval] = diffenvlop(fnewposvals, foriguprval,d_firstdif, d_enddif, d_trc_len, foriguprloc, fnanperwhtnoiseb, fnewposvalsb, fdiffpercentcut, edmode)
        % inputs are difference, orginal envelope,
        % calc the envelope of the difference
        fnewposvalsb(fnewposvals==0) = 0;   % need to keep the zero points in the difference to help keep the turning points in the dc corrected difference
        %dmax_1st_deriv = diff(fnewposvals);
        dmax_1st_deriv = diff(fnewposvalsb);
        ...
        % add in points that are etra
        figure(4435);
        plot(foriguprloc,fnewposvalsb,'-r'); hold on;
        plot(foriguprloc( [diff(([-1;diff(fnewposvalsb,1)]./[-1;diff(foriguprloc,1)]),1);-1] < 0),fnewposvalsb( [diff(([-1;diff(fnewposvalsb,1)]./[-1;diff(foriguprloc,1)]),1);-1] < 0),'rs','MarkerFaceColor','r','MarkerSize',8);
        hold off;
        ...
        %now try to  make the difference peaks more even by removing the values that would get clipped from the peak value
        %alllocs = seqno([-1;ddiffsign;-1] < 0 | [1;ddiffsign;1] > 0 );
        alllocs = unique(sort([uplocs,downlocs]));
        newinterp = double(makefastinterp1(double(foriguprloc(alllocs)),double(fnewposvalsb(alllocs)),double(foriguprloc)));
        %newinterp2 = double(makefastinterp1(double(foriguprloc(alllocs)),double(fnewposvals(alllocs)),double(foriguprloc)));
        
        if edmode == 0 % upper envelope
            %now try to  make the difference peaks more even by removing the values that would get clipped from the peak value
            
%             figure(3000);
%             plot(foriguprloc, foriguprval,'-om');
%             hold on;
%             %plot(foriguprloc, fnewposvals,'-ok');
%             plot(foriguprloc, fnewposvalsb,'-ok')
%             plot(foriguprloc, newinterp,'-or')

            % calculate the ratio between the orginal difference and the one made from interpolating just the peaks
            diffdiff = double(fnewposvalsb./newinterp);
            diffdiff(isnan(diffdiff)) = 0;
            % now sum the ratios so that they can be averaged between the peaks
            diffdiff = cumsum(diffdiff);
            % now average the ratio between the peaks to normalise for the orginal points not being the right angle triangles that the interpolated points are
            difftoadd = [0;diff(diffdiff(downlocs),1)./(diff((downlocs),1)'-1)];
            difftoadd(isnan(difftoadd)) = 1;
            difftoadd(difftoadd>1.3) = 1.3;
            difftoadd(difftoadd<=0) = 1;
            fnewposvalsb(uplocs) = fnewposvalsb(uplocs).*difftoadd;


%             diffdiff = cumsum(double(newinterp - fnewposvalsb));
%             difftoadd = [0;diff(diffdiff(downlocs),1)];
%             difftoadd(difftoadd<0) = 0;
%             fnewposvalsb(uplocs) =  fnewposvalsb(uplocs)-(difftoadd.*0.5);
%            plot(foriguprloc(uplocs),fnewposvalsb(uplocs),'gs','MarkerFaceColor','g','MarkerSize',10);
            
            %now try to  make the difference peaks more even by removing the values that would get clipped from the peak value
            %            diffdiffb = cumsum(double(newinterp2 - fnewposvals));
            %            difftoaddb = [1;diff(diffdiffb(downlocs),1)];
            %            difftoaddb(difftoaddb<0) = 0;
            %            foriguprval(uplocs) =  foriguprval(uplocs)+(difftoaddb.*0.5);
            
            %fnewposvalsb(uplocs) =  fnewposvalsb(uplocs)-(difftoadd);
            %foriguprval(uplocs) =  foriguprval(uplocs)+(difftoadd);
            
            
            % this step thresholds based on the height and the distance from the previous down point ditance so sort of
            % area, but can be misleading if the point is a sudden drop between two widely spaced points
            fupdifval = foriguprval(uplocs(  (fnewposvalsb(uplocs).*[1;diff(foriguprloc(downlocs))])  > (fdiffpercentcut*mean(fnewposvalsb(uplocs).*[1;diff(foriguprloc(downlocs))])) )) ;
            % alternative is just to use the peaks
            %fupdifval = foriguprval(uplocs(  (fnewposvalsb(uplocs))  > (fdiffpercentcut*mean(fnewposvalsb(uplocs))) )) ;
            ...
            %%fupdiflocs = foriguprloc(uplocs(  (fnewposvalsb(uplocs).*[1;diff(foriguprloc(downlocs))])  > (fdiffpercentcut*mean(fnewposvalsb(uplocs).*[1;diff(foriguprloc(downlocs))])) )) ;
            %plot(foriguprloc(uplocs), fnewposvalsb(uplocs),'ob');
            %plot(foriguprloc(downlocs), fnewposvalsb(downlocs),'om');
            ...
            % previous version on area
            fupdiflocs = foriguprloc(uplocs(  (fnewposvalsb(uplocs).*[1;diff(foriguprloc(downlocs))])  > (fdiffpercentcut*mean(fnewposvalsb(uplocs).*[1;diff(foriguprloc(downlocs))])) )) ;
            % version just on peaks
            %fupdiflocs = foriguprloc(uplocs(  (fnewposvalsb(uplocs))  > (fdiffpercentcut*mean(fnewposvalsb(uplocs))) )) ;
            
            %plot(fupdiflocs, fupdifval,'og');
%            plot(fupdiflocs,fupdifval,'rs','MarkerFaceColor','r','MarkerSize',8);
%            hold off;
        else % lower envelope
            figure(3001);
            %             %
            ...
            hold on;
            %             %%plot(foriguprloc, newinterp,'-or');
            %             plot(foriguprloc, fnewposvals,'-og');
            plot(foriguprloc, fnewposvalsb,'-ok');
            %             %%plot(foriguprloc, newinterp2,'-ob');
            plot(foriguprloc, newinterp,'-or')
            %plot(foriguprloc, foriguprval,'-om');
            %now try to  make the difference peaks more even by removing the values that would get clipped from the peak value
            %diffdiff = cumsum(double(newinterp - fnewposvalsb));
            ...
            % now sum the ratios so that they can be averaged between the peaks
            diffdiff = cumsum(diffdiff);
            %difftoadd = [0;diff(diffdiff(uplocs),1)];
            % now average the ratio between the peaks to normalise for the orginal points not being the right angle triangles that the interpolated points are
            difftoadd = [0;diff(diffdiff(uplocs),1)./(diff((uplocs),1)'-1)];
            %difftoadd(difftoadd>0) = 0;
            difftoadd(isnan(difftoadd)) = 1;
            difftoadd(difftoadd>1.3) = 1.3;
            difftoadd(difftoadd<=0) = 1;
            %fnewposvalsb(downlocs) = fnewposvalsb(downlocs)-(difftoadd.*0.5);
            fnewposvalsb(downlocs) = fnewposvalsb(downlocs).*difftoadd;
            plot(foriguprloc(downlocs),fnewposvalsb(downlocs),'gs','MarkerFaceColor','g','MarkerSize',10);
            %foriguprval(downlocs) =  foriguprval(downlocs)+(difftoadd.*0.5);
            
            %now try to  make the difference peaks more even by removing the values that would get clipped from the peak value
            %            diffdiffb = cumsum(double(newinterp2 - fnewposvals));
            %            difftoaddb = [1;diff(diffdiffb(uplocs),1)];
            %            difftoaddb(difftoaddb>0) = 0;
            %            foriguprval(downlocs) =  foriguprval(downlocs)+(difftoaddb.*0.5);
            
            
            %fnewposvalsb(downlocs) =  fnewposvalsb(downlocs)-(difftoadd);
            %foriguprval(downlocs) =  foriguprval(downlocs)+(difftoadd);
            
            %plot(foriguprloc, foriguprval,'-og');
            %plot(foriguprloc, fnewposvalsb,'-ok');
            
            % version based on area
            fupdifval = foriguprval(downlocs(  (fnewposvalsb(downlocs).*[1;diff(foriguprloc(uplocs))])  < (fdiffpercentcut*mean(fnewposvalsb(downlocs).*[1;diff(foriguprloc(uplocs))])  ) )) ;
            %fupdifval = foriguprval(downlocs(  (fnewposvalsb(downlocs).*[1;diff(foriguprloc(uplocs))])  < (fdiffpercentcut*trimmean(fnewposvalsb(downlocs).*[1;diff(foriguprloc(uplocs))],10)  ) )) ;
            % version just on peak
            %fupdifval = foriguprval(downlocs(  (fnewposvalsb(downlocs))  < (fdiffpercentcut*mean(fnewposvalsb(downlocs))) )) ;
            ...
            % version based on area
            %fupdiflocs = foriguprloc(downlocs(  (fnewposvalsb(downlocs).*[1;diff(foriguprloc(uplocs))])  < (fdiffpercentcut*trimmean(fnewposvalsb(downlocs).*[1;diff(foriguprloc(uplocs))],10) ) )) ;
            fupdiflocs = foriguprloc(downlocs(  (fnewposvalsb(downlocs).*[1;diff(foriguprloc(uplocs))])  < (fdiffpercentcut*mean(fnewposvalsb(downlocs).*[1;diff(foriguprloc(uplocs))]) ) )) ;
            % version based on peaks
            %fupdiflocs = foriguprloc(downlocs(  (fnewposvalsb(downlocs))  < (fdiffpercentcut*mean(fnewposvalsb(downlocs))) )) ;
            
            %plot(fupdiflocs, fupdifval,'og');
            plot(fupdiflocs,fupdifval,'rs','MarkerFaceColor','r','MarkerSize',8);
            title(' green squares - peak differences with area shifts applied; red squares - output points; magenta - previous iteration lower envelope, red difference only at peaks in difference ; black - raw difference');
            hold off;
        end
        % make sure the start and end do not get lost
    ...
    function [toriglocs, torigvals] = outsidetest(toriglocs,torigvals,tnewlocs,tnewvals,florigcountb,ulmode,whitemul,diffaddmul)
        ...
        % calc the envelope of the difference
        dmax_1st_deriv = diff(tnewvals);
        dsign_1_deriv = sign(dmax_1st_deriv);
        dsign_1_deriv(dsign_1_deriv == 0) = 1;
        ddiffsign = diff(dsign_1_deriv);
        
        seqno = 1:1:size(tnewlocs,1);
        uplocs = seqno([-1;ddiffsign;-1] < 0);
        downlocs = seqno([1;ddiffsign;1] > 0);
        if size(downlocs,2) > size(uplocs,2)
            downlocs = downlocs(:,1:size(uplocs,2));
        ...
        %newinterp = double(makefastinterp1(double(tnewlocs),double(tnewvals),double(toriglocs)));
        ...
        if ulmode == 0 % upper envelope
            ...
            %         else
            %now try to  make the envlope peaks more even by shifting the values that would get clipped from the peak value
            diffreal = double(newinterp(toriglocs) - torigvals);
            diffreal(diffreal>0) = 0;
            %diffdiff = cumsum(diffreal);
            diffdiff = double(makefastinterp1(double(toriglocs),double(cumsum(diffreal)),double(florigcountb)));
            difftoadd = [diff(diffdiff(tnewlocs(uplocs)),1);0];
            
            %             figure(3003);
            %             plot(toriglocs, torigvals,'-k');
            %             hold on;
            %             %plot(toriglocs, newinterp(toriglocs),'-ob');
            %             plot(tnewlocs(downlocs),tnewvals(downlocs),'gs','MarkerFaceColor','g','MarkerSize',8);
            %             plot(tnewlocs, tnewvals,'-g');
            
            tnewvals(downlocs) =  tnewvals(downlocs)-(difftoadd*diffaddmul);
            
            %             plot(tnewlocs, tnewvals,'--om');
            %hold off;
            ...
            %diffdiff = cumsum(diffreal);
            diffdiff = double(makefastinterp1(double(toriglocs),double(cumsum(diffreal)),double(florigcountb)));
            difftoadd = [diff(diffdiff(tnewlocs(downlocs)),1);0];
            
            %             figure(3004);
            %             plot(toriglocs, torigvals,'-k');
            %             hold on;
            %             plot(toriglocs, newinterp(toriglocs),'-ob');
            %             plot(tnewlocs(uplocs),difftoadd,'gs','MarkerFaceColor','g','MarkerSize',8);
            %             plot(tnewlocs(uplocs),tnewvals(uplocs),'rs','MarkerFaceColor','r','MarkerSize',8);
            %             plot(tnewlocs, tnewvals,'-g');
            
            tnewvals(uplocs) =  tnewvals(uplocs)-(difftoadd*diffaddmul);
            
            %             plot(tnewlocs, tnewvals,'--m');
            %hold off;
    ...
    function [toriglocs, torigvals] = outsidetest_data(toriglocs,torigvals,tnewlocs,tnewvals,florigcountb,ulmode,whitemul,e_firstdif, e_enddif, e_trc_len)
        %[itpslocsin, itpsval] = outsidetest_data(origcount,trim_data(1:noofrealsamps,ckk),itpslocsin,itpsval,origcount,0,(nanperwhtnoise),firstpos, endpos, intrlen);
        
        % toriglocs,torigvals - these are the locactions and values of the previous envelope
        % tnewlocs,tnewvals - these are the locactions and values of the current envelope
        %test to see if any points have been chopped off in the envelope
        %florigcountb - this is the points of the whole trace or just the sequenctial array which is the size of the orginal envelope
        ...
        % interpolate the envelope on to the data sampling
        newinterp = double(makefastinterp1(double(tnewlocs),double(tnewvals),double(florigcountb)));
        
        if ulmode == 0 % upper envelope
            %now try to  make the difference peaks more even by removing the values that would get clipped from the peak value
            %diffdiff = cumsum(double(newinterp(toriglocs) - torigvals));
            %difftoadd = [0;diff(diffdiff(uplocs),1)];
            %difftoadd(difftoadd>0) = 0;
            ...
            diffreal(diffreal>0) = 0;
            diffdiff = cumsum(diffreal);
            difftoadd = [0;diff(diffdiff(tnewlocs),1)];
            
            %figure(3003);
            %plot(toriglocs, torigvals,'-k');
            %hold on;
            %plot(toriglocs, newinterp,'-ob');
            %plot(tnewlocs,difftoadd,'gs','MarkerFaceColor','g','MarkerSize',8);
            %plot(tnewlocs, tnewvals,'-g');
            
            tnewvals =  tnewvals-(difftoadd);
            ...
            %fnewposvalsb(downlocs) =  fnewposvalsb(downlocs)-(difftoadd.*0.5);
            %foriguprval(downlocs) =  foriguprval(downlocs)+(difftoadd.*0.5);
            %tnewvals(downlocs) =  tnewvals(downlocs)+(difftoadd.*0.5);
            %tnewvals(uplocs) =  tnewvals(uplocs)+(difftoadd.*0.5);
            
            %make sure the start and end do not get lost
            if isempty(tnewvals) ~= 1
                if tnewlocs(end) ~= e_trc_len
                    tnewlocs = [tnewlocs; e_trc_len];
                    tnewvals = [tnewvals; e_enddif];
                elseif tnewvals(end) ~= e_enddif
                    tnewvals(end) = e_enddif;
                end
                if tnewlocs(1) ~= 1
                    tnewlocs = [1; tnewlocs];
                    tnewvals = [e_firstdif; tnewvals];
                end
            end
            ...
            %hold on;
            %plot(toriglocs, newinterp,'-ob');
            %%plot(tnewlocs(downlocs),difftoadd,'rs','MarkerFaceColor','r','MarkerSize',8);
            %%plot(tnewlocs(uplocs),difftoadd,'gs','MarkerFaceColor','g','MarkerSize',8);
            %plot(tnewlocs,difftoadd,'gs','MarkerFaceColor','g','MarkerSize',8);
            %plot(tnewlocs, tnewvals,'-g');
