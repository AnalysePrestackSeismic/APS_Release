...
function [] = ig_chi_plot_synthetic( int_dir,maxz,win_chi,inc_chi,int_dp_lim,grad_dp_lim,post_process_flag)
...
%close all;
%tic;
job_meta = struct;                          % make up the job meta information
job_meta.output_dir = strcat(int_dir,'/chiangle');
job_meta.liveblocks = (1:35)';

angles = (1:35)';
job_meta.s_rate = 2000;
syngath =  struct;
int = struct;                                           % Intialize structure for intercept
grad= struct;                                           % Intialize structure for gradient
padding = 5;                                            % padding for the water bottom picker
wbconst = 0;
block='1';


% read all the synthetic traces
syngath.filename=strcat(int_dir,'/Zoe_',block,'_zoe_CJ.sgy');       % Construct the file name , path for the intercept file
...
[syngath.meta syngath.ilxl_bytes syngath.traces]=segy_to_mat('189','193',syngath.filename);                                 % Load the intercept file for the block
syngath.ilxl_bytes = syngath.ilxl_bytes(1,:);
syngath.traces = syngath.traces(:,1)

for icj=2:size(job_meta.liveblocks,1)
    block=num2str(job_meta.liveblocks(icj));
    syngath.filename=strcat(int_dir,'/Zoe_',block,'_zoe_CJ.sgy');       % Construct the file name , path for the synthetic file

    [cjdrop syntmpilxl_bytes syntmptraces]=segy_to_mat('189','193',syngath.filename);                                 % Load the intercept file for the block

    syngath.ilxl_bytes = [syngath.ilxl_bytes; syntmpilxl_bytes(1,:)];
    syngath.traces = [syngath.traces syntmptraces(:,1)];

    clear syntmpilxl_bytes syntmptraces cjdrop
end

%cpa_directory = strcat('/data/TZA/segy/2013_pweza_chewa_presdm_pgs/gather_for_eage/digi_noQ/crossplot_analysis','_',maxz,'_',win_chi,'_',inc_chi,'_',int_dp_lim,'_',grad_dp_lim,'_',post_process_flag,'/'); % The name of the crossplot analysis directory

%syngath.filename = '/data/TZA/rokdoc/Episkopou/Pweza/Gather_100_water_Pweza_1.sgy';
syngath.filename = '/data/TZA/rokdoc/Episkopou/FW1/Virtual_Stack_fw_WATER_1.sgy';
%syngath.filename = '/data/TZA/rokdoc/Episkopou/Gather_CJ_Pweza_1.sgy';
[syngath.meta syngath.ilxl_bytes syngath.traces]=segy_to_mat('189','193',syngath.filename);
syngath.traces = syngath.traces(1400:2100,:);
angles = (1:size(syngath.traces,2));


cpa_directory = strcat(job_meta.output_dir,'crossplot_analysis_new','_',maxz,'_',win_chi,'_',inc_chi,'_',int_dp_lim,'_',grad_dp_lim,'_',post_process_flag,'/'); % The name of the crossplot analysis directory

block=job_meta.liveblocks(1);

%=============================================================================================================================
%
% generate the I& G traces and add the il xls from the input

[inttmp,gradtmp] = ig_reconstruct_linefit(syngath.traces,angles);

% int.ilxl_bytes = [int.ilxl_bytes; inttmpilxl_bytes];
% grad.ilxl_bytes = [grad.ilxl_bytes; gradtmpilxl_bytes];
int.traces = inttmp;
grad.traces = gradtmp;

% [int_sample_p int_sample_t int_trace_p int_trace_t]=peaks_and_troughs(int.traces(:));         % Find peaks and troughs in intercept trace
% [grad_sample_p grad_sample_t grad_trace_p grad_trace_t]=peaks_and_troughs(grad.traces(:));
% %
% % figure(111); plot(int.traces);
% % hold on;
% % scatter(int_sample_p,int_trace_p);
% % scatter(int_sample_t,int_trace_t);
hold off
...
% % end
% 
% wb_n_samples = size(int.traces,1);
% inttmptraces = zeros(size(int.traces,1),size(int.traces,2),'single');
% gradtmptraces = zeros(size(int.traces,1),size(int.traces,2),'single');
% 
% % get the input gather and apply shift to flattern to the water bottom
% for kk = 1:size(int.traces,2);
%     inttmptraces(1:(wb_n_samples-wb_idx(kk)+1),kk) = int.traces(wb_idx(kk):wb_n_samples,kk);
%     gradtmptraces(1:(wb_n_samples-wb_idx(kk)+1),kk) = grad.traces(wb_idx(kk):wb_n_samples,kk);
% end
% 
% int.traces = inttmptraces;
% grad.traces = gradtmptraces;
% 
% clear inttmptraces gradtmptraces
% %% ==========================================================================================================================
...
intercept_temp = int.traces(:,(sum(int.traces,1)~=0|sum(grad.traces,1)~=0));      % Eliminate the blank columns
%intercept_temp(1:wb_sample+wbconst,:)=0;                                               % Mute everything till the waterbottom
gradient_temp =  grad.traces(:,(sum(grad.traces,1)~=0|sum(int.traces,1)~=0));     % Eliminate the blank columns
%gradient_temp(1:wb_sample+wbconst,:)=0;                                                % Mute everything till the waterbottom
intercept_temp = intercept_temp';
...
%                 end
        ...
        if(sum(ll)>(percentile_keep/100)*nsamples_th)
            ...
            if crossplot==1
                ...
                scatter(intercept_ic_uw(ll),gradient_ic_uw(ll))
                %pcolor(hist_axis{2},hist_axis{1},hist_all(1:n_bin,1:n_bin));                                                                                                    % plot the 2D histogram against the bin centres
                hold on;
                plot([-0.2 0.2],(slope_z(ic)*[-0.2 0.2]+grad0),'-k');                                                                                                      % Plot the trendline
                %xlim([-150 150]);
                %ylim([-300 300]);
                hold off;
            ...
            if crossplot==1
                ...
                figure(3);
                scatter(int_trace_pts(ll_pts),grad_trace_pts(ll_pts));
                ...
                %
                hold on;
                plot([0.2 -0.2],(slope_z(ic)*[0.2 -0.2]+grad0),'-k','LineWidth',3);                                                                              % Plot the trendline
                %                 %xlim([-150 150]);
                %                 %ylim([-300 300]);
                %                 plot([0.2 -0.2],(slope_z_p(ic)*[0.2 -0.2]+grad0_z_p(ic)),'-r','LineWidth',3);
                %                 plot([0.2 -0.2],(slope_z_t(ic)*[0.2 -0.2]+grad0_z_t(ic)),'-g','LineWidth',3);
                hold off;
                ...
                figure(4);
                scatter(int_trace_p(ll_p),grad_trace_p_proj(ll_p));
                ...
                hold on;
                plot([0.2 -0.2],(slope_z(ic)*[0.2 -0.2]+grad0),'-k','LineWidth',3);                                                                              % Plot the trendline
                plot([0.2 -0.2],(slope_z_p(ic)*[0.2 -0.2]+grad0_z_p(ic)),'-g','LineWidth',3);
                %                 %xlim([-150 150]);
                %                 %ylim([-300 300]);
                plot([0.2 -0.2],(slope_z_t(ic)*[0.2 -0.2]+grad0_z_t(ic)),'-m','LineWidth',3);
                ...
                figure(5);
                scatter(int_trace_t(ll_t),grad_trace_t_proj(ll_t));
...
%% Store the results in a systematic fashion
...
if plot_result==1
    ...
    for plot_inc=1:7
        ...
        plot(results_AVA.data(plot_inc,:),z_axis,'-r','LineWidth',2); set(gca,'YDir','rev');                                % Plot depth trend of all points in red
        hold on;
        plot(results_AVA.data_pts(plot_inc,:),z_axis,'-b','LineWidth',2); set(gca,'YDir','rev');                            % Plot depth trend of all peaks and troughs in blue
        hold on;
%        plot(results_AVA.data_p(plot_inc,:),z_axis,'-g','LineWidth',2); set(gca,'YDir','rev');                            % Plot depth trend of all peaks in green
%        hold on;
%        plot(results_AVA.data_t(plot_inc,:),z_axis,'-m','LineWidth',2); set(gca,'YDir','rev');                            % Plot depth trend of all tropughs in magenta
        
        
        ylim([min(z_axis) max(z_axis)]);
        switch  plot_inc
            case 1
            ...
            case 5
                ...
                %xlim([-50 50]);
            case 6
                plot(chi_z_emperical,z_axis,'--k','LineWidth',1);
                title('Chi Trend');
                %xlim([-20 20]);
            case 7
...
fit_type = 1;                                                       % Define type of fitting to intercept Gradient Crossplot (values 1 , 2 or 3)
