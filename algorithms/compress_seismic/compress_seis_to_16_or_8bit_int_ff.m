function [ trim_data_filt, scalars_out, zlocs_out, tpgrad_orig, scalepeak,  orig_nsamples, percenterr ] = compress_seis_to_16_or_8bit_int_ff( trim_data, morecompress, envlplot )
%
%% ------------------ Disclaimer  ------------------
%
...
elseif morecompress == 2
    tpgrad_orig = floor(intrlen/15);   % max number of scalars to store
    if tpgrad_orig < 100
        tpgrad_orig = 100;
    end
    extrapoints = floor(intrlen/150); % max number of extra pointas to hold to take care of points missed in derivative envelope
    if extrapoints < 6
        extrapoints = 6;
    end
    nanperwhtnoise  = 0.003;  % % to add to avoid cliping
    %perwhtnoise = 0.06;
    scalepeak = double(90); % the max value to scale to
...
%=========================================================================
        ...
        %if envlplot == 9
            ...
            plot(orig2lowlocs,newnegvals,'-or')
        ...
        % need to make this a measure of the relative difference between
        % the points and orginal points as well as adding some % of the
        % difference
        % the percentage difference should be used to high grade which are
        % the most important points to keep, need to do this for top and
        % bottom
...
%         %========================================================================
%         % add the two sets of points together - test to see which are the
%         %largest gaps and fill those first, but only upto the number of
%         %spare slots
%         tmpupper = origcount;
%         tmpupper(mitpslocsin) = mitpsval;  % populate times sequence with the orginal point values
%         tmpupper(lowdiflocs) = lowdifval;  % populate times sequence with the orginal point values
