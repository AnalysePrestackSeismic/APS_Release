...
iter = 300;         % maximum number of iterations in the lsq solver 
tol = 1e-3;         % convergence tolerance in the lsq solver can move to tol = 5e-4 or 1e-4...
...
wsmooth = job_meta.stdev_smo(str2double(i_block))*5;    % Get stdev for this block from the mat file
...
if job_meta.is_gather == 1
    ...
    % high amplitude
    %taper = [ zeros(floor(padding/2),1)',linspace(0,1,floor(padding*2))];
    %taper = single([ zeros(floor(padding/2),1)',linspace(0,1,floor(padding*2))])';
    ...
    taper = single([ zeros(floor(padding-5),1)',linspace(0,1,20)]);
    vol_traces(:,1:length(taper),:) = bsxfun(@times,vol_traces(:,1:length(taper),:),taper);
    %vol_traces(:,1:(floor(padding/2)+floor(padding*2)),:) = bsxfun(@times,vol_traces(:,1:(floor(padding/2)+floor(padding*2)),:),taper);
...
if ilxl_read{vol_index_wb}(kk,1) == requiredinline;
    ...
    % Do the inversion
    if background == 1;
        [ava_tmp,lsqflag,~] = lsqr(IGiter,data,tol,iter,[],[],model);
        ...
        [ava_tmp,lsqflag,~] = lsqr(IGiter,data,tol,iter,[],[]);
        %ava(:,kk) = single(ava_tmp);
