function [] = wavelet_estimation_var(job_meta_path,i_block,decimate,first_live_block)
%% ------------------ Disclaimer  ------------------
%
...
for i_vol = 1:1:i_vol_max
    ...
    if n_traces > 2
        ...
        for ii = 1:n_win
            ...
            tmpfft = zeros(2,2);
            crop_trace=traces(start_index(ii):end_index(ii),:);
            tmpfft = abs(fft(bsxfun(@times,crop_trace,taperapply)));
            variance_log(i_vol,ii) = var(crop_trace(:));
