function [] = int_grad_inv_proj_lite(seismic_mat_path,i_block,n_blocks,wavelet_mat_path,output_dir)
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
...
% Authors: Jonathan Edgar and James Selvage
    ...
    output_std = 0;
     ...
     for ii = 1:seismic.nfiles
        if ii == 1
            seismic.filepath = '/data/CHN/segy/cgh_DIGI/BG6316-AN05-15FINAL_SMALL';
            [traces{ii}, ilxl_read{ii}] = node_segy_read_traces_lite(seismic,i_block,n_blocks,dec);
        else
            seismic.filepath = sprintf('/data/CHN/segy/cgh_DIGI/BG6316-AN%d-%dFINAL_SMALL',(ii*10)-5,(ii*10)+5);
            [traces{ii}, ilxl_read{ii}] = node_segy_read_traces_lite(seismic,i_block,n_blocks,dec);
        end
    end 
    
    wb_idx = water_bottom_flatten_lite(traces{4});
    %wb_idx = zeros(size(traces{4},2));
    ...
    for ii = 1:seismic.nfiles
        ...
        traces{ii} = traces{ii}(min(wavelet_z_grid)-mode(diff(wavelet_z_grid'))+1:max(wavelet_z_grid)+mode(diff(wavelet_z_grid'))+1,:);
    end
    ...
    alpha = 0; % Weight for EER constraint
...
%         data = bsxfun(@times,data,taper);
        ...
        chi = seismic.srate*(0:1:ns-1)'.*-0.5 + 21;
