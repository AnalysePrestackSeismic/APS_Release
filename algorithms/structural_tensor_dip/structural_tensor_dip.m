function structural_tensor_dip(job_meta_path,i_block,vol_index,sigma,scale_sigma,start_slab,end_slab)
%% ------------------ Disclaimer  ------------------
% 
...
scale_sigma = str2double(scale_sigma);
...
fprintf('-- Loaded block %d --\n',str2double(i_block));
[dipx,dipy,planarity,cline,cplane,cfault] = calculate_st(vol_3d,sigma,scale_sigma);

% Trim volumes to area without aperture
il_min_keep = job_meta.block_keys(str2double(i_block),1)+job_meta.aperture;
il_max_keep = job_meta.block_keys(str2double(i_block),2)-job_meta.aperture;
xl_min_keep = job_meta.block_keys(str2double(i_block),3)+job_meta.aperture;
xl_max_keep = job_meta.block_keys(str2double(i_block),4)-job_meta.aperture;
...
node_segy_write(results_out,str2double(i_block),job_meta.s_rate/1000,[job_meta.output_dir,'/cluster/']);
...
% need to either add flag to job_meta or remove later
% add_aperture_to_job_meta(job_meta_path,num2str(-1*str2num(aperture)));

end

function [Iyy,Iyz,Ixx,Ixz,Ixy,Izz] = calculate_st(vol_3d,sigma,scale_sigma)
% [dipx,dipy,planarity,cline,cplane,cfault] = calculate_st(vol_3d,sigma,scale_sigma)
...
%     dipy(i_loop,1) = atand(ny/nz);
    [l1,l2,l3,nx,ny,nz,~,~,~,~,~,~] = EigenVectors3D(Ixx(i_loop), Ixy(i_loop), Ixz(i_loop), Iyy(i_loop), Iyz(i_loop), Izz(i_loop));
    Ixx(i_loop) = (l1-l2)./l1; % planarity
    Ixy(i_loop) = (l1-l2)./(l1+l2); % cplane
    Ixz(i_loop) = (l2-l3)./(l2+l3); % cline
    Iyy(i_loop) = atand(nx/nz); % dipx
    Iyz(i_loop) = atand(ny/nz); % dipy
end

% cfault = (1-cplane).*cline;
Izz = (1-Ixy).*Ixz; % cfault
