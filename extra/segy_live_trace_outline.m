function [qcslice,live_trace_polygon] = segy_live_trace_outline(il_loc_byte,xl_loc_byte,xloc_byte,yloc_byte,filename)
%% ------------------ Disclaimer  ------------------
% 
...
il_loc_byte = str2double(il_loc_byte);                               
xl_loc_byte = str2double(xl_loc_byte); 
...
out_file_name = strcat(filename,'_poly');
...
qcslice = zeros(4,int_n_traces,'int64');               % decalre the array for a couple of time slices
...
%         %         tracesc = single(tracesc1.*tracesc2.*tracesc3);
    ...
    trace_ilxl_bytes(:,3) = int64(trace_header(bytes_to_samples == il_loc_byte,:))';
    trace_ilxl_bytes(:,4) = int64(trace_header(bytes_to_samples == xl_loc_byte,:))';
    ...
    qcslice(1:4,(((ii-1)*blocktr)+1):ii*blocktr) = [int64(trace_ilxl_bytes(:,1)');int64(trace_ilxl_bytes(:,2)');int64(trace_ilxl_bytes(:,3)');int64(trace_ilxl_bytes(:,4)')];
...
%         temparr2 = [reshape(typecast(trchead(:),'int32'),[],blocktr); reshape(typecast(tracesout(:),'int32'),strseismic.n_samples,blocktr)];
%         
...
if leftovers > 0
    ...
    % make a couple of matlab arrays to hold a couple of time slices
    qcslice(1:4,(((ii)*blocktr)+1):end) = [int64(trace_ilxl_bytes(:,1)');int64(trace_ilxl_bytes(:,2)');int64(trace_ilxl_bytes(:,3)');int64(trace_ilxl_bytes(:,4)')];
...
%         
%         %reattach the raw headers with a reshape to float32
%         temparr2 = [reshape(typecast(trchead(:),'int32'),[],leftovers); reshape(typecast(tracesout(:),'int32'),strseismic.n_samples,leftovers)];
%         
%         %and write out as big endian ieee segy, have to write ebcdic first though
%         %fwrite(poly_ieee_segy,temparr2,'int32',0,'ieee-be');
%     end
    ...
    slice_x(lin_ind) = qcslice(1,:)';
    slice_y(lin_ind) = qcslice(2,:)';
