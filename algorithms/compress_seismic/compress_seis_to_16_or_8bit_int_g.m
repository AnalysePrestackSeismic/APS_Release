...
for ckk = 1:no_of_traces
    ...
    if sum(trim_data(:,ckk)) ~= 0;
        ...
        for envl = 1:nl
            ...
            if size(mitpslocsin,1) < floor(tpgrad/2);
                break;
            end
        end
...
%         end
    ...
    function [fupdiflocs, fupdifval] = diffenvlop(fnewposvals, foriguprval,d_firstdif, d_enddif, d_trc_len, foriguprloc, fnanperwhtnoiseb, ftotalenvdist, fdiffpercentcut, edmode)
        % calc the envelope of the difference
...
%             if fdiffpercentcut > 0 % drop points that are below a threshold size
%                 ftotalenvdistval = abs(fnewposvals([-1;ddiffsign;-1] < 0))./ftotalenvdist([-1;ddiffsign;-1] < 0);
%                 fupdiflocs = fupdiflocs(ftotalenvdistval > fdiffpercentcut);
%                 fupdifval = fupdifval(ftotalenvdistval > fdiffpercentcut);
%             end
        else % lower envelope
...
%             if fdiffpercentcut > 0 % drop points that are below a threshold size
%                 ftotalenvdistval = abs(fnewposvals([-1;ddiffsign;-1] > 0))./abs(ftotalenvdist([-1;ddiffsign;-1] > 0));
%                 fupdiflocs = fupdiflocs(ftotalenvdistval > fdiffpercentcut);
%                 fupdifval = fupdifval(ftotalenvdistval > fdiffpercentcut);
%             end            
        end        
