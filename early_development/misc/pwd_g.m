...
% phase term.
    ...
    for jj = 1:n_traces-1
        ...
        for ii = 1:iter_max
            ...
            % best fit for dm=(inv(Cd*D))*-C*d under the constraint that
            % diff(dm)=0.
...
% associated filter of derivatives needed for the nonlinear inversion.
    ...
    C = [spdiags([c11,c21,c31],[0,1,2],n_samples,2+n_samples),spdiags([c12,c22,c32],[0,1,2],n_samples,2+n_samples)];
    ...
    Cd = [spdiags([cd11,cd21,cd31],[0,1,2],n_samples,2+n_samples),spdiags([cd12,cd22,cd32],[0,1,2],n_samples,2+n_samples)];
end
