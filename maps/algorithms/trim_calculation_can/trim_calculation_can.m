function [] = trim_calculation_can(job_meta_path,i_block,startvolr,endvolr,shift,zsmooth,itmr,otmr,timbal,outputflatgathers,varargin)
%% ------------------ Disclaimer  ------------------
% 
...
%   shift:              Maximum allowable shift in samples (Not used currently ? Automatically calculated from local frequency content)
...
%     resamp_rt > (sample_rate * 1.8)
    samp_drop = 1;%round(resamp_rt/(sample_rate*1.8) + 0.1); set it to 2 if you want to drop every other sample. saves computation time
    sample_rate = sample_rate * samp_drop;
    ufoff = 1000/(sample_rate*2);
    ufon = ufoff*0.8;
    n_samples = length(1:samp_drop:n_samples);
...
%             trim_data_temp= [fliplr(trim_data) trim_data];
%             trim_data_temp2 = medfilt3nt(trim_data_temp,[0 5],0);
%             trim_data=trim_data_temp2 (:,size(trim_data,2)+1:2*size(trim_data,2));
        ...
        if max(max(trim_data_filt))==0 || isnan(max(max(trim_data_filt)))
            fprintf('Empty or Null traces present in block\n');
        end
        if timbal == 1 
...
%             figure(11);
%             subplot(3,1,1); plot(xcorva);grid on;
...
%            if plots == 1; figure(206); imagesc((-(idxshift-1):1:(idxshift-1))*shiftinc,1:1:450,det_coefb(1:end,:)); caxis([0.8 1]); hold all; plot((idxb(1:450)-idxshift)*shiftinc,1:1:450,'bs','MarkerEdgeColor','b','MarkerFaceColor','b'); xlabel('Shift'); ylabel('Sample No.'); title('small smooth xcorrelation map'); end;
            ...
            % *** insert the filtering of trimshifts by xcor to prevent
            % uncorrelated event to jump too much
...
%                 subplot(3,1,3); plot(t_shift);grid on;
                t_shift_b(xcorva_smooth<xcor_th)=0;
            end
        ...
        clear t_shift t_shiftb xcorva_smooth;
        ...
        if plots == 1;
            ...
            plotinb = [fliplr(tmp_plotin),tmp_plotout];% FLip left to right
...
%         norm_fold(norm_fold == 0) = 1;        
        ...
        if plots == 1;  
            ...
            plot(((trim_sum/max(trim_sum)).*2+(size(plotinb,2)/2.1)),1:1:length(trim_sum),'bs','MarkerEdgeColor','b','MarkerFaceColor','b');
        end
