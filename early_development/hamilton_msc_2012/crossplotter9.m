...
function[] = crossplotter9(Yaxisfile,Xaxisfile, nt,ntwin,bytespersample, divisions, run_number, no_xlines,no_inlines, no_cross_bins,no_inline_bin)
fprintf('\n Reading in Data..\n');
...
hop = (no_xlines);
binsize_in = round(no_inlines/no_inline_bin)
starter = 0
count3 = 1
jump= 1;
...
buffer(:,1) = fread(fid1,no_samples_slice*nt,'float32');   
buffer2(:,1)= fread(fid2,no_samples_slice*nt,'float32');
starter = 0;
finish = 0; 
nt_iterated = 0;
tic 
...
while finish ~= 1 
    ...
    inline_bin_counter = inline_bin_counter + 1;
     
    
    
    if mod(b,no_xlines) == 0 
    toc
    starter = starter+(binsize)+1  
    inline_number_iterated = inline_number_iterated + 1 
    
    tic
    if inline_number_iterated == (no_inlines-binsize_in)
    starter = starter + (binsize_in*no_xlines)
    inline_number_iterated = 0;
    nt_iterated = nt_iterated+1;
    end 
    if nt_iterated == (nt - ntwin)
    finish = 1;
    end 
    else               
    starter = starter +4;
