function[] = small_volume_PCA(Yaxisfile,Xaxisfile, nt,ntwin,bytespersample, divisions, run_number, no_xlines,no_inlines, no_cross_bins,no_inline_bin)


% Function to calculate the background trend using PCA around each data
% point in a seismic volume. It may not be optimised for any volume and
%therefore only may work on the test volume(size 300inx650xl). 

%% INPUT

% Yaxisfile = crossplot y axis data file in slice optimised
% Xaxisfile = crossplot x axis data file in slice optimised
% nt = number of time samples
% ntwin = length of time window
% divisions = how oftend do you want to sample the data. This may be a
% redundant attribute so by default use the same number as nt.
% run_number = dummy variaible used to label output files with a number
% no_xlines = no xline in volume
% no_inlines= no inlines in volume 
% no_cross_bins = number of xline bins
% no_inline_bin = number of inline bins


%% OUTPUT

% 'run_number_flat_bk_trend_angle' file containing the angle in each
% sample. 
% 
