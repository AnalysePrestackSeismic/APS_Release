...
elseif morecompress == 2
    ...
    if tpgrad_orig < 40 % was 55
        tpgrad_orig = 40; % was 55
    end
    ...
    nanperwhtnoise  = 0.007;  % % to add to avoid cliping
    %perwhtnoise = 0.06;
    scalepeak = double(127); % the max value to scale to
...
for ckk = 1:no_of_traces
    ...
    if sum(trim_data(:,ckk)) ~= 0;
        ...
        if or(and( percenterr(ckk) > 0.25, morecompress > 2), and( percenterr(ckk) > 0.001, morecompress <= 2))
