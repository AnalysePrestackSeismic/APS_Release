function [] = node_segy_write_traces(results_out,i_block,output_dir)
% -------------------------------------------------------------------------
% NODE_SEGY_WRITE_TRACES: Function to rapidly write trace ordered data to
% SEGY. The "results_out" format is a cell array which always contains
% metadata in the first row. Subsequent rows contain data matricies to be
% written.
%   Inputs:
%       results_out = a cell array with two colums, the left containing
%       labels which are used to name output SEGY files and the right
%       containing the data to be written:
%
%                   'il_xl numbers'     ILXL_numbers(cell array)
%                   'digi_intercept'    digi_intercept(cell array)
%
%       i_block = current block for writing. output_dir = directory to
%       write SEGY files to.
%   Outputs:
%       n_blocks of SEGY files.
% Authors: Charles Jones and Simon Wrigley.
% -------------------------------------------------------------------------

n_results = size(results_out,1);

for i_results = 2:1:n_results % top row in results is meta inforation
    
    file_name = strcat(output_dir,sprintf('%s_result_block_%d%s',results_out{i_results,1},i_block,'.segy'));
    ...
    for ii= 1:block_sz:size(results_out{i_results,2},2)
        ...
        temparr2(48,:) = typecast(results_out{1,2}{1,1}(ii:maxblock,1)','int32');
