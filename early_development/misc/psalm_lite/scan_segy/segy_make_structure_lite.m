function [seismic] = segy_make_structure_lite_js(filepath,filename,il_byte,xl_byte,varargin)
%%-------------------------------------------------------------------------
...
%%-------------------------------------------------------------------------
    ...
    for i_file = 1:1:nfiles        
        ...
        file_mat = strcat(filepath,file_mat{1},'.mat_lite');
        ...
        if scan_exist == 0
            ...
            fid_write = fopen(strcat(file_mat),'w');
    ...
    %         fwrite(fopen(strcat(file_mat,'_lite'),'w'),[seismic.file_type;seismic.n_samples;reshape(seismic.trace_ilxl_bytes',[],1)],'double');
    %         cd(start_point)        
        end 
        clearvars tmptrheader trace_header bytes_to_samples trace_ilxl_bytes
    end
