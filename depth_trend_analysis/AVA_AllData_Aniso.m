...
[WellI,las_hdr]=read_las2_file('Well_I_DepthTrends.las',0);
...
WellJ.curves=downsample(WellJ.curves,N);
...
VsGas=Vs(Diag==Comp & Sw <= SndSwCO);
VsWtr=Vs(Diag==Comp & Sw > SndSwCO);
RhoGas=Rho(Diag==Comp & Sw <= SndSwCO);
RhoWtr=Rho(Diag==Comp & Sw > SndSwCO);

% Overburden facies
VpSlt=Vp(Vsh >= SndVshCO & Vsh < SltVshCO & PhiE < SndPorCO);
VpShl=Vp(Vsh >= SltVshCO & PhiE < SndPorCO);
VsSlt=Vs(Vsh >= SndVshCO & Vsh < SltVshCO & PhiE < SndPorCO);
VsShl=Vs(Vsh >= SltVshCO & PhiE < SndPorCO);
RhoSlt=Rho(Vsh >= SndVshCO & Vsh < SltVshCO & PhiE < SndPorCO);
RhoShl=Rho(Vsh >= SltVshCO & PhiE < SndPorCO);
Pob=P(Vsh >= SltVshCO & PhiE < SndPorCO);
...
Pp=mean(P)*0.006895; % Average Pf for fluid property calculation (MPa)
...
PhiTob=NEU(Vsh >= SltVshCO & PhiE < SndPorCO);
ini(MaxZ,1)=0;
...
for bb=1:length(Zsb);
    for aa=1:length(Z);
        ...
        Phydro(aa,bb)=1000*0.000145*9.81*((RhoW*Z(aa))+(RhoSea*Zsb(bb)));
        %Create Lithostatic curve
        Plith_f(aa,bb)=(0.000145*9.81)*(ini(aa,1)+(Zsb(bb)*RhoSea*1000));
    end
end
...
VsGas_filt=Vs(Vsh <= Sand_hi & Vsh >= Sand_low & Diag==Comp & Sw <= SndSwCO & Pover < Pco);
VsWtr_filt=Vs(Vsh <= Sand_hi & Vsh >= Sand_low & Diag==Comp & Sw > SndSwCO & Pover < Pco);
RhoGas_filt=Rho(Vsh <= Sand_hi & Vsh >= Sand_low & Diag==Comp & Sw <= SndSwCO & Pover < Pco);
RhoWtr_filt=Rho(Vsh <= Sand_hi & Vsh >= Sand_low & Diag==Comp & Sw > SndSwCO & Pover < Pco);
...
PhiT_filt_slt=NEU(Vsh >= SndVshCO & Vsh < SltVshCO & PhiE < SndPorCO & Age>GA_low & Age<GA_hi & Pover < Pco);
PhiT_filt_shl=NEU(Vsh >= SltVshCO & PhiE < SndPorCO & Age>GA_low & Age<GA_hi & Pover < Pco);
RhoShl_filt=Rho(Vsh >= SltVshCO & PhiE < SndPorCO & Age>GA_low & Age<GA_hi & Pover < Pco);
VpShl_filt=Vp(Vsh >= SltVshCO & PhiE < SndPorCO & Age>GA_low & Age<GA_hi & Pover < Pco);
VsShl_filt=Vs(Vsh >= SltVshCO & PhiE < SndPorCO & Age>GA_low & Age<GA_hi & Pover < Pco);
RhoSlt_filt=Rho(Vsh >= SndVshCO & Vsh < SltVshCO & PhiE < SndPorCO & Age>GA_low & Age<GA_hi & Pover < Pco);
VpSlt_filt=Vp(Vsh >= SndVshCO & Vsh < SltVshCO & PhiE < SndPorCO & Age>GA_low & Age<GA_hi & Pover < Pco);
VsSlt_filt=Vs(Vsh >= SndVshCO & Vsh < SltVshCO & PhiE < SndPorCO & Age>GA_low & Age<GA_hi & Pover < Pco);
...
if Shale == 0
    ...
    RhoOB_filt=Rho(Vsh >= SndVshCO & Vsh < SltVshCO & PhiE < SndPorCO & Age>GA_low & Age<GA_hi & Pover < Pco);
    VpOB_filt=Vp(Vsh >= SndVshCO & Vsh < SltVshCO & PhiE < SndPorCO & Age>GA_low & Age<GA_hi & Pover < Pco);
    VsOB_filt=Vs(Vsh >= SndVshCO & Vsh < SltVshCO & PhiE < SndPorCO & Age>GA_low & Age<GA_hi & Pover < Pco);
...
I1=max(A_Rand_wtr_aniso);
G1=min(B_Rand_wtr_aniso);
...
MinEERgas=A_Pred_gas_aniso*Iscalar*cos(Chi_min_r)+B_Pred_gas_aniso*Gscalar*sin(Chi_min_r);
MinEERwtr=A_Pred_wtr_aniso*Iscalar*cos(Chi_min_r)+B_Pred_wtr_aniso*Gscalar*sin(Chi_min_r);
...
MinEERgas_rnd=A_Rand_gas_aniso.*Iscalar.*cos(Chi_min_r)+B_Rand_gas_aniso.*Gscalar.*sin(Chi_min_r);
MinEERwtr_rnd=A_Rand_wtr_aniso.*Iscalar.*cos(Chi_min_r)+B_Rand_wtr_aniso.*Gscalar.*sin(Chi_min_r);
...
MinlEERgas_rnd=A_Rand_gas_aniso.*Iscalar.*cos(Chi_minl_r)+B_Rand_gas_aniso.*Gscalar.*sin(Chi_minl_r);
MinlEERwtr_rnd=A_Rand_wtr_aniso.*Iscalar.*cos(Chi_minl_r)+B_Rand_wtr_aniso.*Gscalar.*sin(Chi_minl_r);
...
plot(A_Rand_gas_aniso*Iscalar,B_Rand_gas_aniso*Gscalar,'o','MarkerSize',2,'Parent',subplot1,'MarkerFaceColor','red','LineWidth',1,'MarkerEdgeColor','red');
plot(A_Rand_wtr_aniso*Iscalar,B_Rand_wtr_aniso*Gscalar,'o','MarkerSize',2,'Parent',subplot1,'MarkerFaceColor','blue','LineWidth',1,'MarkerEdgeColor','blue');
plot(A_Pred_wtr_aniso*Iscalar,B_Pred_wtr_aniso*Gscalar,'o','MarkerSize',10,'Parent',subplot1,'MarkerFaceColor','blue','LineWidth',4,'MarkerEdgeColor','black');
...
plot(A_Pred_gas_aniso*Iscalar,B_Pred_gas_aniso*Gscalar,'o','MarkerSize',10,'Parent',subplot1,'MarkerFaceColor','red','LineWidth',4,'MarkerEdgeColor','black');
...
% Plot Data
scatter(A_Rand_gas_aniso*Iscalar,B_Rand_gas_aniso*Gscalar,100,Por_r_gas,'.','Parent',subplot1)
...
% Plot Data
scatter(A_Rand_wtr_aniso*Iscalar,B_Rand_wtr_aniso*Gscalar,100,Por_r_wtr,'.','Parent',subplot2)
