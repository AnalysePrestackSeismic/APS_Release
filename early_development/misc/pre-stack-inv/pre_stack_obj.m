function f = pre_stack_obj(angle_stacks,prior,weight,wavelet,angles,imp_in)
...
%imp(:,1) = p(1:n_samples)';
%imp(:,2) = p(n_samples+1:end)';

% term 1
% generate synthetics
del_imp = zeros(n_samples,n_angles);

imp(:,1) = imp_in(1:length(prior(:,1)));
imp(:,2) = imp_in(length(prior(:,1))+1:end);
