function [] = trim_calculation(job_meta_path,i_block,startvolr,endvolr,speedup,zsmooth,itmr,otmr,timbal,outputflatgathers,varargin)
%% ------------------ Disclaimer  ------------------
%
...
startvolr_bk=startvolr;
endvolr_bk=endvolr;
zsmooth_bk=str2double(zsmooth);
useselectemode = 0;
var_resample=0;
...
tmpoffset = double(unique(offset_read));
offset = unique(sort(tmpoffset'));

%tmpoffset = tabulate(offset_read);
%offset = tmpoffset(tmpoffset(:,2) ~= 0,1);

% Check if the fold calculated is not the same as number of unique offset
% found in the data
if offset_fold ~= size(offset,1)
    %tmp2offset = offset([diff(offset,1);2] > 1);
    offinc = mode(diff(offset([diff(offset,1);2] > 1),1));
    tmp2offset = offset([diff(offset,1);2] == offinc);
...
%if (offset(1) <= startvolr)
    ...
    startvol = find(offset >= startvolr,1,'first');
    ...
    fprintf('\n Provided startvolr invalid. This should be >= %d and less than %d\n',offset(1),endvolr);
    error('Error: Wrong Input parameters');
end
...
end_str =strcat(num2str(startvolr_bk),'_',num2str(endvolr_bk),'_',num2str(zsmooth_bk)); % Create the appending strink for naming directories
% prestack output
filename = 'gaths';
results_out{2,1} = strcat(filename,'_trim_data_',end_str);
%results_out{2,2} = zeros(n_samples,n_traces_gather,n_traces,'single');
%commented out as written to above
results_out{2,3} = 1;

if outputgathershifts == 1;
    results_out{3,1} = strcat(filename,'_trim_shifts_',end_str);
    results_out{3,3} = 1;
...
% stack result
filename2 = 'stack';
results_out2{2,1} = strcat(filename2,'_mica_sum_',end_str);
results_out2{2,2} = zeros(in_n_samples,n_traces,'single');
results_out2{3,1} = strcat(filename2,'_mica_vals_good_data_',end_str);
...
results_out2{4,1} = strcat(filename2,'_mica_xcor_',end_str);
results_out2{4,2} = zeros(in_n_samples,n_traces,'single');
results_out2{5,1} = strcat(filename2,'_posttrim_',end_str);
results_out2{5,2} = zeros(in_n_samples,n_traces,'single');
results_out2{6,1} = strcat(filename2,'_pretrim_',end_str);
    ...
    if results_out{1,2}{1,1}(kk,1) == requiredinline;
        ...
        if isfield(job_meta,'mute') && kk==1
            ...
            for m1=1:size(job_meta.mute,1)
                %check is data starts after defined inner mute
                if(offset(1)<job_meta.mute(m1,2))
