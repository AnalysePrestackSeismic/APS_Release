function [] = ava_analysis_mica( job_meta_path,block,int_dir,grad_dir,win_chi,inc_chi,int_dp_lim,grad_dp_lim,varargin)
%% ------------------ Disclaimer  ------------------
% 
...
%   win_chi = 400;       % Calculation moving window for chi calculation (in m or ms)
%   inc_chi = 50;
...
%   varagin{1} = Optional input of xcor_mica threshold if there is one and yo
%   want to use mica_xcor to constrain events used in crossplot
%   varagin{2} = Full path f mica xcor directory
...
cpa_directory = strcat(job_meta.output_dir,'crossplot_analysis','_',maxz,'_',win_chi,'_',inc_chi,'_',int_dp_lim,'_',grad_dp_lim,'/'); % The name of the crossplot analysis directory
...
xcor_th = str2double(varargin{1})/100;  % COnvert xcor threshold form string to number and then from percentage to fraction
...
if length (intercept)<3
    % if the intercept trace has too few points a meaning ful regression
    % cant be done. Assign default -999 values to all the output
    int_mean_z=-999;
    grad_mean_z=-999;
    int_std_z=-999;
    grad_std_z = -999;
