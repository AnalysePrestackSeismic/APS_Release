...
for bb=1:nfiles;
    ...
    Plith(1) = 1000*1E-6*9.81*sum(RhoL(1:round(well_data{bb}.Depth_TVDml(1)))); 
    ...
    wc=1000*1E-6*9.81*well_data{bb}.Zsb*RhoSea;
    ...
    for aa=1:length(well_data{bb}.Depth_TVDml)-1;
            ...
            well_data{bb}.Plith(aa,1) = (Plith(aa)+wc);
    end
...
% Lithostatic
Plith_pred=145.0377*(1000*1E-6*9.81)*(sum(RhoL(1:Zpred))+(Zwc*RhoSea));
% Hydrostatic
...
RhoGas_dt = RhoQ-((RhoQ-mRhoGas)*exp(-betaPor*Zrange));
...
VpGas_dt = VpQ-((VpQ-mVpGas)*exp(-betaPor*Zrange));
...
VsGas_dt = VsQ-((VsQ-mVsGas)*exp(-betaPor*Zrange));
...
RhoWtr_dt = RhoQ-((RhoQ-mRhoWtr)*exp(-betaPor*Zrange));
...
VpWtr_dt = VpQ-((VpQ-mVpWtr)*exp(-betaPor*Zrange));
...
VsWtr_dt = VsQ-((VsQ-mVsWtr)*exp(-betaPor*Zrange));
...
[VpGas_dt]=Overpressure(a_sst,b_sst,c_sst,Pves_pred,Pop,Buoy,VpGas_dt,mVsGas,VpQ);
[VsGas_dt]=Overpressure(a_sst,b_sst,c_sst,Pves_pred,Pop,Buoy,VsGas_dt,mVsGas,VsQ);
[RhoGas_dt]=Overpressure(a_sst,b_sst,c_sst,Pves_pred,Pop,Buoy,RhoGas_dt,mRhoGas,RhoQ);
...
[VpWtr_dt]=Overpressure(a_sst,b_sst,c_sst,Pves_pred,Pop,0,VpWtr_dt,mVsWtr,VpQ);
[VsWtr_dt]=Overpressure(a_sst,b_sst,c_sst,Pves_pred,Pop,0,VsWtr_dt,mVsWtr,VsQ);
[RhoWtr_dt]=Overpressure(a_sst,b_sst,c_sst,Pves_pred,Pop,0,RhoWtr_dt,mRhoWtr,RhoQ);
