...
nt = 100;                                              % trace length in samples
...
%intervals as you like
  ...
  wedge(2,:)=[60,55,50,45,40];
  wedge(1,1:(length(wedge(2,:))))=37; 
...
LEGEND('real I data','non-conditioned estimate','conditioned data estimate','DIGI')
    if save_on == 1
    set(gcf,'PaperPositionMode','auto')
    savefig('iso_wedge_rw I','pdf','tiff');
    saveas(figure(3),'iso_wedge_rw I.fig');
    hgsave('iso_wedge_rw I','-v7.3');
    end
figure(4)
hold
...
ylabel('Value of G','FontSize',axis_title_fontsize)
    if save_on == 1
    set(gcf,'PaperPositionMode','auto')
    savefig('iso_wedge_rw G','pdf','tiff');
    saveas(figure(4),'iso_wedge_rw G.fig');
    hgsave('iso_wedge_rw G','-v7.3');
    end