function [] = seismic_anomaly_spotter_dtect(job_meta_path,vol_index,window_length,amp,hilb,start_slab,end_slab)
%% ------------------ Disclaimer  ------------------
% 
...
if isfield(job_meta, 'wb_path')
    wb_idx_in = dlmread(job_meta.wb_path);                      % Read the water bottom file
...
while lpi <= loopfin
    ...
    %wb_idx = ones(1,size(traces,2));
     if isfield(job_meta, 'wb_path')
        ...
        if sum(zero_loc) > 2          
            ...
            %else
            %    wb_idx = wb_idx_in(locations(zero_loc),3);                
            %end            
        ...
        %   wb_idx(:,3) = wb_idx(:,3)./(job_meta.s_rate/1000);
            ...
            traces = traces(:,x(1):x(end));
            ilxl_read = ilxl_read(x(1):x(end),:);
            ...
            traces = zeros(size(traces,1),size(traces,2));
            wb_idx = ones(1,size(traces,2));
            win_sub = bsxfun(@plus,wb_idx,(0:job_meta.n_samples{str2double(vol_index)}-1)');
    ...
    if str2double(hilb) == 1 && str2double(amp) == 0 
        traces = imag(hilbert(traces)); % this will perform a -90 phase rotation
    ...
    if lpi == 1;
        ...
        if ~exist([job_meta.output_dir,'water_bottom.xyz'],'file')
            write_wb = 1;
...
if ~isfield(job_meta, 'wb_path')
   job_meta.wb_path = [job_meta.output_dir,'water_bottom.xyz'];
