function [] = wavelet_estimationandQC_CAN(job_meta_path,i_block,decimate,first_live_block)
%% ------------------ Disclaimer  ------------------
%
...
% Make directory for  saving pre Inversion QC Products
if exist(strcat(job_meta.output_dir,'preQC/'),'dir') == 0
    mkdir(strcat(job_meta.output_dir,'preQC/'))
end
qc_directory = strcat(job_meta.output_dir,'preQC/');

%% WATER BOTTOM RELATED STUFF
...
if isfield(job_meta, 'wb_path')
    wb_idx_in = dlmread(job_meta.wb_path,'delimiter',',');
%     wb_import = importdata(job_meta.wb_path);
%     wb_idx_in(:,1) =wb_import.data(:,1) ;
%     wb_idx_in(:,2) =wb_import.data(:,2) ;
%     wb_idx_in(:,3) =wb_import.data(:,4) ;
    % col 1 inline
    % col 2 xline
    % col 3 twt
...
for i_vol = 1:1:i_vol_max
    ...
    if n_traces > 2
        ...
        if size(traces,1)>1100
            trace_temp = traces(100:1100,:);
        else
            trace_temp = traces(100:end,:);
        end
        variance(i_vol) = var(trace_temp(:)); % this has been added for Canada since the zone below wb is very high amplitude
        clear trace_temp;
        ...
        start_index_qc = 1:ns_win_qc-ns_overlap_qc-1:n_samples-ns_win_qc;
        end_index_qc = start_index_qc+ns_win_qc-1;
        n_win_qc = length(start_index_qc);
        if i_vol==1
            QC_AVA=struct;
            QC_AVA.rms = zeros(i_vol_max,n_win_qc);
            QC_AVA.var = zeros(i_vol_max,n_win_qc);
        end
        %
        ...
        for ii = 1:n_win
            ...
            tmpfft = zeros(2,2);
            traces_crop=traces(start_index(ii):end_index(ii),:);
            tmpfft = abs(fft(bsxfun(@times,traces_crop,taperapply)));
        ...
        for ii = 1:n_win_qc
            ...
            traces_crop=traces(start_index_qc(ii):end_index_qc(ii),:);
            QC_AVA.rms(i_vol,ii)=rms(traces_crop(:));
            QC_AVA.var(i_vol,ii)=var(traces_crop(:));
        ...
        if i_vol == 1
            ...
            QC_AVA.i_vol_max=i_vol_max;
            QC_AVA.n_win=n_win_qc;
            QC_AVA.pick_wb_ind=pick_wb_ind;
            QC_AVA.wb_ind_avg=wb_ind_avg;
        ...
        if i_vol==i_vol_max
            qc_filepath = strcat(qc_directory,'ava_qc_',i_block,'.mat');
            save(qc_filepath,'-struct','QC_AVA','-v7.3');
        end
    end
...
if n_traces > 2
    ...
    % stdev = 4;  %edited for CAN
    % might like to think about calculating this properly based on live samples
