...
sal=23230; %ppm
og=34; % oil gravity (API no)
gg=0.56; % gas gravity (Specific gravity)
giib=1; % Gas Index in Brine
giio=1; % Gas Index in Oil
GOR=500;
gor=GOR*0.177295; % Gas Oil Ratio (L/L)
So=0.8; % Oil saturation for oil case
Sg=0.0; % Gas saturation for oil case
So1=0; % Oil saturation for gas case 
Sg1=0.8; % Gas saturation for gas case
...
[Kreusso(f),rhoeffo(f),Kvoigto(f),vpb(f),rhob(f),Kb(f),vpo(f),rhoo(f),Ko(f),vpg(f),rhog(f),Kg(f),gor(f)]=flprop(Method(f),Sal(f),OG(f),GG(f),GOr(f),GIIB(f),GIIO(f),Phy(f),Tp(f),SO(f),SG(f));
[Kreussg(f),rhoeffg(f),Kvoigtg(f),vpb(f),rhob(f),Kb(f),vpo(f),rhoo(f),Ko(f),vpg(f),rhog(f),Kg(f),gor(f)]=flprop(Method(f),Sal(f),OG(f),GG(f),GOr(f),GIIB(f),GIIO(f),Phy(f),Tp(f),SO1(f),SG1(f));
end
...
KREUSSo=1e9*Kreusso;
KREUSSg=1e9*Kreussg;
KSAND(1:MaxZ)=1e9*K_sand;
RHOB=1e3*rhob;

[VpOil_fs(:,N),VsOil_fs(:,N),RhoOil_fs(:,N),k2] = gassmnv_mb(VpWtr_trend,VsWtr_trend,1e3*RhoWtr_trend,RHOB,KB,RHOEFFo,KREUSSo,KSAND,Por_trend(:,N));
[VpGas_fs(:,N),VsGas_fs(:,N),RhoGas_fs(:,N),k2] = gassmnv_mb(VpWtr_trend,VsWtr_trend,1e3*RhoWtr_trend,RHOB,KB,RHOEFFg,KREUSSg,KSAND,Por_trend(:,N));
...
cellarrayout{16,1} = 'Rho_oil_sand';
cellarrayout{16,2} = RhoOil_fs/1000;
cellarrayout{17,1} = 'Vp_oil_sand';
cellarrayout{17,2} = VpOil_fs;
cellarrayout{18,1} = 'Vs_oil_sand';
cellarrayout{18,2} = VsOil_fs;
cellarrayout{19,1} = 'Porosity_sand';
cellarrayout{19,2} = Por_trend;
cellarrayout{20,1} = 'Sand_Facies';
cellarrayout{20,2} = Sand;
