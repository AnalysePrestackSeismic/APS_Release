function [] = ni_pr_inv_proj(job_meta_path,i_block)
% -------------------------------------------------------------------------
...
wsmooth = 1000;
eer_weight = 0; % Weight for EER constraint
iter = 100;
tol = 1e-2;
...
results_out{1,2}{2,1} = uint32(zeros(size(traces{1},2)));
results_out{2,1} = 'digi_ni';
results_out{2,2} = digi_intercept;
results_out{3,1} = 'digi_pr';
...
if exist(strcat(job_meta.output_dir,'digi_results_ni_pr/','intercept_gradient_segy/'),'dir') == 0
    output_dir = strcat(job_meta.output_dir,'digi_results_ni_pr/','intercept_gradient_segy/');
    mkdir(output_dir);    
else
