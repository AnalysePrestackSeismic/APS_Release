function [] = sw_segy_write_lite_gathers(results_out,i_block, sample_rate, output_dir, varargin)
...
for i_results = 2:1:n_results % top row in results is meta inforation includes output directory (can either be local to node or on system)
    ...
    for ii= 1:block_sz:size(results_out{i_results,2},2)
        ...
        temparr2(48,:) = results_out{1,2}{1,i_results-1}(ii:maxblock,1)'; % assumes inline / crossline are the same for all files
