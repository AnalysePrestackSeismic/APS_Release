...
function [] = ig_plot( job_meta_path,int_dir,grad_dir,maxz,win_chi,inc_chi,int_dp_lim,grad_dp_lim,post_process_flag)
...
wbconst = 0;
block=num2str(job_meta.liveblocks(1));
...
for icj=2:size(job_meta.liveblocks,1)
    ...
    [cjdrop inttmpilxl_bytes inttmptraces]=segy_to_mat('189','193',int.filename);                                 % Load the intercept file for the block
    [cjdrop gradtmpilxl_bytes gradtmptraces]=segy_to_mat('189','193',grad.filename);                             % Load the gradient file for the block  
    int.ilxl_bytes = [int.ilxl_bytes; inttmpilxl_bytes];
    int.traces = [int.traces inttmptraces];
    grad.ilxl_bytes = [grad.ilxl_bytes; gradtmpilxl_bytes];
    grad.traces = [grad.traces gradtmptraces];
    clear inttmpilxl_bytes inttmptraces gradtmpilxl_bytes gradtmptraces cjdrop
end
    ...
    if (max(abs(intercept_ic_uw_raw))>0 && max(abs(gradient_ic_uw_raw))>0)
        ...
        if(sum(ll)>(percentile_keep/100)*nsamples_th)
            ...
            if crossplot==1
                ...
                hold on;
                plot(hist_axis{2},(slope_z(ic)*hist_axis{2}+grad0),'-k');                                                                                                      % Plot the trendline
                xlim([-150 150]);
                ylim([-300 300]);
                hold off;
...
%                plot(hist_axis_pts{2},(slope_z_t(ic)*hist_axis_pts{2}+grad0_z_t(ic)),'-y','LineWidth',3); 
                hold off;
