...
%[~,nfiles] = (system('ls -B | wc -l'));    %linux
%nfiles=str2double(nfiles);  %linux
nfiles=size(dir('*.sgy'),1); %JVRT


% Read all filenames and convert from ascii to double
%[~,fnames] = system('ls -B1');  %linux
%numeric = double(fnames);  %linux
fnames=dir('*.sgy'); %JVRT
files_in.names={fnames.name}';  %JVRT


% Preallocate memory for some variables LINUX
%count = 2;
%fname_index = zeros(1,nfiles+1);

% Loop to separate out each file name from one long character string LINUX
...
% Loop to read each file as ascii into a cell array LINUX
%for i = 1:nfiles
