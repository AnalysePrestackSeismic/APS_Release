function [] = int_grad_inv_proj_CAN(job_meta_path,i_block,startvol,volinc,endvol,tottracerun,maxzout,wavevar)
%
%% ------------------ Disclaimer  ------------------
% 
...
if use_spatial_wavelets == '0'                                                      % Use single wavelet set
    ...
    wavelets = load_wavelet_spatial_CAN(job_meta_path,wavelet_dir_path,i_block,algo);   % Call function to smoothen wavelet spatially by various algorithms and load the wavelet for this block
end
%%
...
%results_out{4,2} = digi_minimum_energy_eer_projection;
digi_maximum_energy_eer_projection = [bsxfun(@times,ava(1:ns,:),cosd(90-chi))+bsxfun(@times,ava(1+ns:end,:),sind(90-chi));zeros(job_meta.n_samples{vol_index_wb}-ns,ntraces)];
results_out{resultno,2} = zeros(job_meta.n_samples{vol_index_wb},ntraces);
% Unflatten data using the window index
