...
%     tracezeros = (trim_data(:,ckk) ~= 0).*origcount;
...
%     midblank = zeros(noofrealsamps,1,'double');
%     scaleval = scaleval(1:noofrealsamps);
%     midpoints = midblank;
%     itpslocs = single(1:intrlen)';
    ...
    if itpslocsin(end) > mitpslocsin(end)
        ...
        %itpsval = [itpsval; (trim_data(end,ckk)+(itpsval(end)-trim_data(end,ckk))*0.25)];
    end
...
%     points_all = points_all([1;diff(points_all)] > 0);
    ...
    if envlplot > 2
        ...
        %figure(102); plot((abs((double(trim_data(:,ckk)) - double(diffout)))./abs(double(trim_data(:,ckk)))).*100,'-k');
        %          hold on;
    ...
    percenterr = (abs(sum(abs(double(trim_data(:,ckk)))) - sum( abs( double(diffout))))/sum(abs(double(trim_data(:,ckk)))))*100;
...
%     summofinput = sum(abs(trim_data(:,ckk)));
    ...
    if or(and( percenterr > 0.05, morecompress > 2), and( percenterr > 0.25, morecompress == 2))
