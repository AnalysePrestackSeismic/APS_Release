function job_meta_path = segy_make_job_js(filepath,filename_string,il_byte,xl_byte,...
    offset_byte,parallel,anggath,output_dir,varargin)
%% ------------------ Disclaimer  ------------------
% 
...
% Angle Gather: gather_name_block1.sgy where 1 is the volume number and would be incremented if there are multiple segy files per volume
...
%         end
%         
%         for f_key = 1:1:size(byte_loc2,1) % this might not be looping through all required files? is this size on the wrong thing! meaning 437 is correct.l
...
%             seismic = segy_read_binary(strcat(job_meta.paths{1},job_meta.files{f_key}));
...
%             write_file = strcat(job_meta.paths{1},strrep(job_meta.files{f_key},'.mat_orig_lite','.mat_lite'));
...
if ~isempty(varargin)
    str_extra = ['_',varargin{1}];
else
    str_extra = '';
end

job_meta_path = strcat(job_meta_dir,'job_meta_',str_date,str_extra,'.mat');
