function [] = int_grad_inv_proj_lite_sw(seismic_mat_path,i_block,n_blocks,wavelet_mat_path,output_dir)
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
...
% add function to read seismic binary scan and meta .mat file
...
%remove hard coding below - take srate from scansegy
seismic.srate = 0.004;
seismic.nfiles = 8;
seismic.angles = [7.5,12.5,17.5,22.5,27.5,32.5,37.5,42.5]; %make this user defined or from meta file

load(wavelet_mat_path);

% temp added by SRW
wavelet = all_wavelets_time;
%
...
for ii = 1:seismic.nfiles
    if ii == 1
        seismic.filepath = '/data/Global/dtect/SRW_test_dataset/SEGY/05-10_angle_stack_SRW.sgy';
        ...
        seismic.filepath = sprintf('/data/Global/dtect/SRW_test_dataset/SEGY/%d-%d_angle_stack_SRW.sgy',ii*5,(ii+1)*5);
...
%% Optional section to write out wb_idx data for unflattening the SAS results

fid_wb = fopen(strcat(output_dir,'waterbottom/waterbottom_idx_block_',num2str(i_block),'.bin'),'w');
fwrite(fid_wb,wb_idx,'float32');
fclose(fid_wb);
...
% fwrite(fid_out,digi_minimum_energy_eer_projection','float32');
% fclose(fid_out);
...
results_out{4,1} = 'digi_minimum_energy_eer_projection'; % < remove projection from SEGY file
results_out{4,2} = digi_minimum_energy_eer_projection;
if output_std == 1;
    results_out{5,1} = 'std_intercept';
    results_out{5,2} = std_intercept;
    results_out{6,1} = 'std_gradient';l
